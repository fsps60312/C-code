...
int SetRoot(const int root)
{
	...
	for(int i=0;i<(int)ET[root].size();i++)
	{
		...
		for(int l=0,r=(int)tpath.size()-1;l<(int)tpath.size();l++)
		{
			while(r>=0&&tpath[r]+tpath[l]>K)r--;
			if(r<0)break;
			ta-=r+1;
		}
...
//	for(const int v:path)printf("%d ",v);puts(""); 
	for(int l=0,r=(int)path.size()-1;l<(int)path.size();l++)
	{
		while(r>=0&&path[r]+path[l]>K)r--;
		if(r<0)break;
		ta+=r+1;
	}
	assert(ta%2==0);
	ans+=ta/2;
	for(int i=0;i<(int)path.size()&&path[i]<=K;i++)ans++;
	return ans;
}
int main()
{
