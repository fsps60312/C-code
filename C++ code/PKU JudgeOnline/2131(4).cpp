...
#include<vector>
using namespace std;
void assert(const bool &valid){if(valid)return;int i=0,j=0;i/=j;for(int *a;;a--)(*a)=20151023;}
int N,M;
struct Node
{
	Node *ch[2];
...
{
	if(!o){assert(loc==0);/*sth went wrong here*/o=new Node(v);return false;}
	if(loc<=Sz(o->ch[0]))
	{
...
//	freopen("out.txt","w",stdout);
	...
	{
//		assert(N>=1&&N<=131072&&M>=1&&M<=131072);
		ROOT=NULL;
		BuildTree(ROOT,1,M-2);
