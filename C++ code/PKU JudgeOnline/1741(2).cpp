...
int SetRoot(const int root)
{
	...
	for(int i=0;i<(int)ET[root].size();i++)
	{
		...
		const int nxt=(e.a==root?e.b:e.a);
		if(VIS[nxt])continue;
		vector<int>*tpath=new vector<int>();
...
//	for(const int v:path)printf("%d ",v);puts(""); 
	...
	{
		int ta=0;
		for(int i=0;i<(int)chpaths.size();i++)for(int j=0;j<(int)chpaths[i]->size();j++)
			ta+=Rank(path,K-(*chpaths[i])[j])-(Rank((*chpaths[i]),K-(*chpaths[i])[j]));
		assert(ta%2==0);
		ans+=ta/2;
	}
	ans+=Rank(path,K);
