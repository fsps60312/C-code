...
struct BccSolver
{
	...
	stack<int>STK;
	int Dfs(const int u,const int pre_ei)
	{
		...
		for(const int ei:ET[u])if(ei!=pre_ei)
		{
			...
			if(!PRE[nxt])
			{
				...
				if(nxt_low>=PRE[u])
				{
					...
					//duplicated edges!!! a-b,a-b,a-b,...
					if((int)edges.size()==1)ANS.push_back(edges[0]),assert(nxt_low>PRE[u]);//get asserted
					else assert(nxt_low==PRE[u]);
				}
			}
	...
	void Solve(const vector<int>&edges)
	{
		...
		{
			const Edge &e=EDGE[ei];
			if(!PRE[DJ[e.a]])Dfs(DJ[e.a],-1),assert(STK.empty());
			assert(PRE[DJ[e.b]]);
		}
	}
}BCC;
