	...
	while(testcount--)
	{
		...
		for(int chance=0,cyclecount=BuildCycles();;opecount++,chance++)
		{
			...
			if(chance>=N-cyclecount)
			{
				...
				for(int i=0,j=0;i<N;i++)if(S[i]!=i)
				{
				...
				delete[]trans;
				delete[]loc;
				assert((int)ans.size()==N-cyclecount);
...
//					printf("(%d->%d)(%d->%d)\n",Leaf(Root(p.first),0)-NODES,Leaf(Root(p.first),1)-NODES,Leaf(Root(p.second),0)-NODES,Leaf(Root(p.second),1)-NODES);
//					printf("(%d->%d)(%d->%d)\n",Leaf(Root(S[p.first]),0)-NODES,Leaf(Root(S[p.first]),1)-NODES,Leaf(Root(S[p.second]),0)-NODES,Leaf(Root(S[p.second]),1)-NODES);
//					printf("(%d->%d)\n",Leaf(Root(p.first),1)-NODES,Leaf(Root(S[p.first]),0)-NODES);
//					puts("a");
...
//					puts("b");
				...
				else
				{
					...
					if(PutDownParents(&NODES[first])>PutDownParents(&NODES[second]))swap(first,second);
					Node *a,*b,*c;
