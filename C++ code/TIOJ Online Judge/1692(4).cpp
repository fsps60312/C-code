...
int main()
{
	...
	for(const vector<int>&path:paths)
	{
		...
		vector<int>ans;
		ans.push_back(path[0]+1);
		for(int i=1;i<(int)path.size();i+=2)
		{
			if(path[i]<M)ans.push_back(path[i]+1);
			else
			{
				ans.push_back(path[i-1]+1);
				anses.push_back(ans);
				ans.clear();
				ans.push_back(path[i+1]+1);
			}
		}
		ans.push_back(path[(int)path.size()-1]+1);
		anses.push_back(ans);
		vector<int>().swap(ans);
	}
	for(const vector<int>&ans:anses)if((int)ans.size()>2)
	{
		ReportVst(ans[0]);
		for(int i=1;i+1<(int)ans.size();i++)ReportE(ans[i]);
		ReportVed(ans[(int)ans.size()-1]);
	}
	Final();
	return 0;
}
