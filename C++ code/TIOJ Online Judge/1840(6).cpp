...
struct Node
{
	Node *ch[2];
	int sz,ref;
	Node(const int _sz):ch{NULL,NULL},sz(_sz),ref(0){Node_COUNTER++;}
	~Node(){Node_COUNTER--;}
};
void Delete(Node* &o)
{
	if(o->ch[0]&&!--(o->ch[0]->ref))Delete(o->ch[0]);
	if(o->ch[1]&&!--(o->ch[1]->ref))Delete(o->ch[1]);
	delete o;
}
...
{
	if(l==r)return new Node(Sz(o)+v);
	const int mid=(l+r)/2;
	Node *ans=new Node(0);
	if(loc<=mid)Assign(ans->ch[0],Modify(Ch(o,0),l,mid,loc,v)),Assign(ans->ch[1],Ch(o,1));
	else Assign(ans->ch[0],Ch(o,0)),Assign(ans->ch[1],Modify(Ch(o,1),mid+1,r,loc,v));
	Maintain(ans);
	return ans;
}
int GetCh0Sum(const vector<Node*>&s){int ans=0;for(Node *o:s)ans+=(o?Sz(o->ch[0]):0);return ans;}
...
struct Bit
{
	...
	void Build()
	{
		for(int i=1;i<=N;i++)Assign(DATA[i],NULL);
	}
