...
{
	const int loc=PutDownParents(o);
...
//	freopen("in.txt","r",stdin);
	...
	while(testcount--)
	{
		...
		int chance=0,cyclecount=BuildCycles();
		for(opecount=0;;opecount++)
		{
//			printf("opecount=%d\n",opecount);
...
//			printf("cycle count=%d\n",cyclecount);
			...
			if(++chance>=N-cyclecount)
			{
				...
				for(int i=opecount;i>=0;i--)swap(trans[OPES[i].first],trans[OPES[i].second]);
				int *loc=new int[N];
