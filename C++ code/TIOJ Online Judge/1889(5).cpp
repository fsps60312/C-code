...
bool InSameTree(const pair<int,int>&a,const pair<int,int>&b){return Dist(a.first,b.first)<a.second+b.second;}
bool Solve()
{
	const vector<pair<int,int> >&ps=LINE[WDIS[0]];
	vector<pair<int,int> >stk,hp;
	for(const auto &p:ps)
	{
		if(stk.empty()||!InSameTree(stk.back(),p))
		{
			stk.push_back(p);
	...
	const pair<int,int>king=stk.back();
	int cnt=0;
	while(!stk.empty())
	{
		if(InSameTree(stk.back(),king))
		{
			if(stk.size()<2){cnt++;break;}
			stk.pop_back(),stk.pop_back(),cnt++;
		}
		else
		{
			if(hp.empty())return false;
			stk.pop_back(),hp.pop_back(),cnt++;
		}
	}
	cnt+=hp.size();
	return cnt*2>N;
