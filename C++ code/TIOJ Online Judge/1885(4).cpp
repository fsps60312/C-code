...
void BestChoice(const vector<int>&s,vector<int>&ans,vector<int>nxts_ans[3])
{
	...
	ans.clear();
	ans.push_back(INF);
//	random_shuffle(THREE_PERMUTS.begin(),THREE_PERMUTS.end());bug!!!
...
//	random_shuffle(FOUR_PERMUTS.begin(),FOUR_PERMUTS.end());bug!!!
...
void orderCoins()
{
	...
	while((int)now.size()>1)
	{
		...
		printf("choice:");for(const int v:choice)printf(" %d",v);puts("");
		int id;
		switch(choice[1])
		{
		...
		for(const int s:now)
		{
			for(const int v:PERMUTATIONS[s])printf("%d ",v+1);
			puts("");
		}
		puts("");
	}
