...
int GetHeight(const vector<int>&s)
{
	...
	if((int)HEIGHT.size()%1000==0)printf("size=%d\n",(int)HEIGHT.size());
	vector<int>choice,nxts[3];
	BestChoice(s,choice,nxts);
	return HEIGHT[s]=choice[0];
}
void BestChoice(const vector<int>&s,vector<int>&ans,vector<int>nxts_ans[3])
{
	ans.clear();
	...
	for(const vector<int>&choice:THREE_PERMUTS)
	{
		...
		for(int type=0;type<3;type++)
		{
			...
			{
				int max_height=0;
				for(int i=0;i<3;i++)getmax(max_height,GetHeight(nxts[type][i]));
				if(max_height<ans[0])
				{
					ans.clear(),ans.push_back(max_height);
					ans.push_back(type);
					for(int i=0;i<3;i++)ans.push_back(choice[i]),nxts_ans[i]=nxts[type][i];
				}
			}
		}
	}
	for(const vector<int>&choice:FOUR_PERMUTS)
	{
		...
		{
			int max_height=0;
			for(int i=0;i<3;i++)getmax(max_height,GetHeight(nxts[i]));
			if(max_height<ans[0])
			{
				ans.clear(),ans.push_back(max_height);
				ans.push_back(3);
				for(int i=0;i<4;i++)ans.push_back(choice[i]),nxts_ans[i]=nxts[i];
			}
		}
	}
}
