...
struct Trie
{
	...
	void SortStrings(vector<string>&s)
	{
		...
		{
			ans.insert(make_pair(RIGT[GetLoc(s[i].c_str())],s[i]));
		}
		s.clear();
		for(const auto &it:ans)s.push_back(it.second);
	}
};
struct QueryType
{
...
//		for(const string s:S)puts(s.c_str());
		...
		for(int i=0;i<M;i++)
		{
			...
			if(loc!=-1)
			{
				...
				QUERY.push_back(QueryType(s.c_str(),TRIE.LEFT[loc]-1,&ANS[i*2]));
				QUERY.push_back(QueryType(s.c_str(),TRIE.RIGT[loc],&ANS[i*2+1]));
			}
		}
		...
		static Trie suf;
		suf.Clear();
		const int backup_TRIE_SZ=TRIE.SZ;
		for(int i=0,j=0,pre_u=0;;i++)
		{
			const int u=(i<N?TRIE.RIGT[TRIE.GetLoc(S[i].c_str())]:INF);
			assert(pre_u<=u);
			pre_u=u;
