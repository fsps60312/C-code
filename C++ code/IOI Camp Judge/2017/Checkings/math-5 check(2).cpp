...
vector<int>CastToFactors(const vector<int>&s,vector<int>*ans,bool power1)
{
	static vector<bool>vis(1000001,false);
	vector<int>id;
	for(int v:s)
	{
		for(const int f:GetFactors(v,power1))
		{
...
LL CountCoprimes(const vector<int>&s)
{
	...
	const auto &id=CastToFactors(s,f,true);
	LL ans=0;
	for(const int i:id)
	{
		const LL cnt=C((int)f[i].size(),K);
...
//	printf("mx=%d\n",mmxx);
	...
	while(testcount--)
	{
		...
		static vector<int>f[1000001];
		const auto &id=CastToFactors(S,f,false);
		LL ans=0;
		for(const int i:id)
		{
//			printf("f = %d, s =",i);
