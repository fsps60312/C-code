...
map<int,vector<int> >CastToFactors(const vector<int>&s,bool power1)
{
	map<int,vector<int> >ans;
	for(int v:s)
	{
		for(const int f:GetFactors(v,power1))ans[f].push_back(v);
	}
	return ans;
}
int N,K;
...
{
	const auto &f=CastToFactors(s,true);
	LL ans=0;
	for(const auto &p:f)
	{
...
//	printf("mx=%d\n",mmxx);
	...
	while(testcount--)
	{
		...
		const auto &f=CastToFactors(S,false);
		LL ans=0;
		for(const auto &p:f)
		{
