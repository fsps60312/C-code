...
vector<int>CastToFactors(const vector<pair<int,int> >&s,vector<pair<int,int> >*ans)
{
	static vector<bool>vis(1000001,false);
	vector<int>id;
	for(const auto &v:s)
	{
		for(const int f:GetFactors(v.first,false))
		{
...
vector<int>CastToFactors(const vector<pair<int,int> >&s,vector<int>&ans)
{
	static vector<bool>vis(1000001,false);
	vector<int>id;
	for(const auto &v:s)
	{
		for(const int f:GetFactors(v.first,true))
		{
			if(!vis[f])vis[f]=true,id.push_back(f);
			ans[f]+=v.second;
		}
...
int N,K;
LL CountCoprimes(const vector<pair<int,int> >&s)
{
...
//	printf("mx=%d\n",mmxx);
	...
	while(testcount--)
	{
		...
		S.clear();
		{
			...
			for(const auto &p:s)S.push_back(p);
		}
		static vector<pair<int,int> >f[1000001];
		const auto &id=CastToFactors(S,f);
		LL ans=0;
		for(const int i:id)
		{
...
//			for(const int v:f[i])printf(" %d",v);puts("");
			vector<pair<int,int> >s;
			for(const auto &v:f[i])s.push_back(make_pair(v.first/i,v.second));
