...
struct Node
{
	...
	void Print()
	{
		printf("(id=%d,p=%d,ch={%d,%d})",id+1,parent==-1?0:ID[parent]+1,ch[0]?ch[0]->id+1:0,ch[1]?ch[1]->id+1:0);
	}
}*S[100000];
...
void Splay(Node *o)
{
	...
	{
//		puts("pass");
		Rotate(Parent(p)&&GetChId(Parent(p),p)!=-1?GetMe(p):(tmp=p),d);
//		assert(ID[p->parent]==ID[o->id]);
//		int tmp=ID[o->parent];
		Rotate(p);
//		assert(ID[o->parent]==tmp);
	}
}
Node *Access(Node *o)
{
...
//		puts("b");
	...
	Splay(pre);
	while(pre->ch[0])pre->PutDown(),pre=pre->ch[0];
	Splay(pre);
	return pre;
}
...
{
	while(Parent(a)&&GetChId(Parent(a),a)!=-1)a=Parent(a);
	while(Parent(b)&&GetChId(Parent(b),b)!=-1)b=Parent(b);
	return a==b;
}
int main()
{
