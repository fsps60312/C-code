...
//		printf("ks[%d]=(%d,%d)\n",i,ks[i].first,ks[i].second);
		...
		while(stk.top().second<=y_lowbound)stk.pop();
...
//			printf("remain=%d,basesum=%d,allsum=%d\n",remain,QuerySum(ltree,rtree,0,N,y_lowbound),QuerySum(ltree,rtree,0,N,N));
...
//	freopen("09","r",stdin);
	scanf("%d",&N);
//	vector<int>t;
//	for(int i=0;i<5;i++)t.push_back(5+i);
//	t.insert(t.begin(),0),t.insert(t.begin(),1);
//	for(const int v:t)printf("%d ",v);return 0;
	BuildPlane();
