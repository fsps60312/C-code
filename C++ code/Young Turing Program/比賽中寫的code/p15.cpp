...
int n,m,up;
int ex[maxn],tim[maxn],l[maxn];
vector<int> id[maxn];
inline int to(int u,int i){return u^ex[i];}
void read()
{
	RI(n,m,up);
	REP(i,m)
...
void build()
{
	...
	dis[1][0]=0;pq.push(H{1,0,0});
	while(SZ(pq))
	{
		while(SZ(pq) && dis[pq.top().u][pq.top().d]!=pq.top().t) pq.pop();
		if(!SZ(pq)) break;
		int u=pq.top().u,t=pq.top().t,d=pq.top().d;pq.pop();
		for(int i:id[u])
		{
			int v=to(u,i);
			if(d+l[i]>up) continue;
			if(dis[v][d+l[i]] > t+tim[i])
			{
				dis[v][d+l[i]]=t+tim[i];
				pq.push(H{v,t+tim[i],d+l[i]});
			}
		}
	}	
}
void sol()
{
	int ans=*min_element(dis[n],dis[n]+maxn);
	if(ans>1e9) PL(-1);
	else PL(ans);
}
int main()
