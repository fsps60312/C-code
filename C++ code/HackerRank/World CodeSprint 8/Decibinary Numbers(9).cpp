...
LL GetBinaryNumber(const LL x)
{
	...
	for(int digitPos=19;digitPos>=0;digitPos--)
	{
		...
		for(int remainder=0;remainder<=18;remainder++)if(rankL_DP[digitPos+1][remainder])
		{
			const LL dp=rankL_DP[digitPos+1][remainder];
			for(int digit=0;digit<=9&&digit<=remainder;digit++)
			{
				const int nxtRemainder=(remainder-digit)*2+1;
				rankL_DP[digitPos][min(nxtRemainder,18)]+=dp;
			}
		}
		LL tmp=0;
		for(int i=0;i<=18;i++)tmp+=rankL_DP[digitPos+1][i]*GetRankR(digitPos+1,i,ans);
//		if(true)
//		{
...
//			puts("a");
			ans-=1LL<<digitPos;
			for(int i=0;i<=18;i++)rankR_DP[digitPos+1][i]=-1,rankL_DP[digitPos][i]=0;
			for(int remainder=0;remainder<=18;remainder++)if(rankL_DP[digitPos+1][remainder])
			{
				const LL dp=rankL_DP[digitPos+1][remainder];
				for(int digit=0;digit<=9&&digit<=remainder;digit++)
