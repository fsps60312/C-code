...
#include<utility>
LL ANS;
void DfsSummarize(const int u,const int parent,const int ql,const int qr,const LL now_cost)
{
	for(const int ki:EK[u])
	{
		const Edge &ticket=TICKET[ki];
		const int nxt=(u==ticket.A?ticket.B:ticket.A);
		SEG.Add(L[nxt],R[nxt],ticket.COST);
//		printf("Add(%lld)\n",ticket.COST);
	}
	getmax(ANS,SEG.QueryMax(ql,qr)-now_cost);
//	printf("u=%d, best=%lld\n",u+1,SEG.QueryMax(ql,qr)-now_cost);
	for(const int ei:ET[u])
	{
		const Edge &e=EDGE[ei];
		const int nxt=(u==e.A?e.B:e.A);
		if(nxt!=parent)DfsSummarize(nxt,u,ql,qr,now_cost+e.COST);
	}
	for(const int ki:EK[u])
	{
		const Edge &ticket=TICKET[ki];
...
Node*Merge(const int u,Node*a,Node*b)
{
	...
	for(const auto &nxt:a->CH)
	{
		DfsSummarize(nxt.first,u,b->L,b->R,nxt.second);
		b->CH.push_back(nxt);
	}
	b->SZ+=a->SZ;
	getmin(b->L,a->L);
...
//	printf("u=%d\n",u);
	...
	for(const int ei:ET[u])
	{
		...
		if(nxt==parent)continue;
		DfsSolve(nxt,u);
		chus.push_back(make_pair(nxt,e.COST));
		SEG.Add(L[nxt],R[nxt],-e.COST);
	}
	Node*p=new Node(0,L[u],L[u]);
	for(const auto &nxt:chus)
	{
		Node*chp=new Node(SZ[nxt.first],L[nxt.first],R[nxt.first]);
...
int main()
{
	...
	while(scanf("%d",&N)==1)
	{
		...
		{int tick=0;BuildSquashedTree(0,-1,tick);assert(tick==N);}
		ANS=0;
		SEG.Build(N);
		DfsSolve(0,-1);
