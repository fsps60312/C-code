...
void BuildSUM(const int id,const int l,const int r,const int bl,const int br)
{
	...
	int smid=-1;
	SUM[id][mid]=INF;
	for(int i=max(id,bl);i<=min(mid-1,br);i++)if(getmin(SUM[id][mid],L[id][i]+R[i+1][mid]))smid=i;
	assert(smid!=-1);
...
//	freopen("in.txt","r",stdin);
	while(scanf("%d%d",&N,&K)==2)
	{
		...
		for(int r=N-1;r>=0;r--)
		{
			...
			for(int l=r-1;l>=0;l--)
			{
				sum+=(D[l+1]-D[l])*(cnt+=S[l+1]);
				L[l][r]=sum;
			}
		}
		for(int l=0;l<N;l++)SUM[l][l]=0,BuildSUM(l,l+1,N-1,l,N-2);
		for(int i=0;i<N;i++)DP[0][i]=R[0][i];
		int d=0;
		for(int i=2;i<=K;i++,d^=1)BuildDP(DP[d],DP[d^1]);
		LL ans=INF;
		for(int i=0;i<N;i++)getmin(ans,DP[d][i]+L[i][N-1]);
		printf("%lld\n",ans);
	}
	return 0;
}
