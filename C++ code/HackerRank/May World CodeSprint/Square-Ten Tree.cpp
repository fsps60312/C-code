...
void Solve(vector<pair<int,vector<int> > >&ansl,vector<pair<int,vector<int> > >&ansr)
{
	...
	if(SAME==1){ansl.push_back(make_pair(0,vector<int>{R[0]-L[0]+1}));return;}
...
//	printf("(%d,%d)\n",(int)ansl.size(),(int)ansr.size());
	for(int move=1,pre_move=0,level=1;;pre_move=move,move<<=1,level++)
	{
		if(SAME<=move+1)
		{
//			puts("pass");
			if(true)
			{
				...
				s.first=level;
//				puts("a");
				for(int i=pre_move+1;i<SAME-1;i++)
				{
...
//				puts("d");
			...
			if(true)
			{
				pair<int,vector<int> >s;
				s.first=level;
				for(int i=pre_move+1;i<SAME-1;i++)s.second.push_back(R[i]-'0'),R[i]='0';
		...
		else
		{
			...
			if(true)
			{
				pair<int,vector<int> >s;
				s.first=level;
				for(int i=pre_move+1;i<=move;i++)s.second.push_back(R[i]-'0'),R[i]='0';
				while(!s.second.empty()&&s.second.back()==0)s.second.pop_back();
				if(!s.second.empty())ansr.push_back(s);
			}
		}
