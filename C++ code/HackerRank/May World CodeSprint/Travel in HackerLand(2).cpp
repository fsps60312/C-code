...
struct DisjointSet
{
	...
	void Clear()
	{
		for(int i=0;i<N;i++)PARENT[i]=i,QIN[i].clear(),CS[i].clear(),CS[i].insert(COLOR[i]),SZ[i]=1+(int)EQ[i].size();
	}
	...
	bool Merge(int a,int b,const int cost)
	{
		...
		for(const int qid:EQ[a])
		{
			...
			if(nxt==b)EQ[nxt].erase(qid),QIN[b].insert(make_pair(q.cost,qid));
