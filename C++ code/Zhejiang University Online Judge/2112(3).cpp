...
struct Node
{
	Node *ch[2];
	int sum;
}BUFFER[1750000],*NEW;//1700000~1800000
...
struct Bit
{
	...
	int Query(const int l,const int r,const int rank)
	{
		...
		const int ans=::Query(los,ros,0,m-1,rank);
		los.clear(),vector<Node*>().swap(los);
		ros.clear(),vector<Node*>().swap(ros);
		return ans;
	}
}BIT;
...
//	freopen("in.txt","r",stdin);
	...
	QUERY.reserve(10005),RANGE.reserve(50005);
	while(testcount--)
	{
		NEW=BUFFER;
