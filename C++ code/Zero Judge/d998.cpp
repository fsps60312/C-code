...
LL Solve(LL ans,const vector<LL>&s)
{
//	printf("ans=%lld\n",ans);
	const int n=s.size();
	for(int b=60,row=0;b>=0&&row<n;b--)if(s[row]&(1LL<<b))
	{
		if(!(ans&(1LL<<b)))ans^=s[row];
		row++;
	}
	return ans;
}
void DeDuplicate(vector<LL>&s)
{
	const int n=s.size();
	...
	for(int i=n-1;i>=0&&s[i]==0LL;i--)s.pop_back();
}
struct DisjointSets
{
	int PARENT[50000];
...
//	freopen("in.txt","r",stdin);
	while(scanf("%d%d",&N,&M)==2)
	{
		...
		DJ.Clear(N);
		vector<LL>cycles;
		for(const Edge &e:EDGE)if(!DJ.Merge(e.a,e.b,e.cost))
		{
			cycles.push_back(DJ.QueryXO(e.a)^DJ.QueryXO(e.b)^e.cost);
		}
//		else
//		{
//			printf("(%d,%d):",e.a,e.b);
//			for(int i=0;i<N;i++)printf("%lld ",DJ.QueryXO(i));puts("");
//		}
//		for(int i=0;i<N;i++)printf("%lld ",DJ.QueryXO(i));puts("");
//		for(const LL v:cycles)printf("%lld ",v);puts("");
		DeDuplicate(cycles);
//		for(const LL v:cycles)printf("%lld ",v);puts("");
