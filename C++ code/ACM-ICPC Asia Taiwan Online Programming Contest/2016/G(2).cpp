...
//				if(nxtlm.find(nxtp.first)!=nxtlm.end())nxtp.second.second=false;
	...
	else
	{
		...
		sort(lm[MAX_HL].begin(),lm[MAX_HL].end());
		puts("ddd");
		sort(rm[MAX_HL].begin(),rm[MAX_HL].end());
		for(auto p=rm[MAX_HL].begin(),nxtp=rm[MAX_HL].begin();p!=rm[MAX_HL].end();p=nxtp)
		{
			nxtp=upper_bound(rm[MAX_HL].begin(),rm[MAX_HL].end(),*p);
			const auto &itl=lower_bound(lm[MAX_HL].begin(),lm[MAX_HL].end(),*p),&itr=upper_bound(lm[MAX_HL].begin(),lm[MAX_HL].end(),*p);
			if(itl!=itr)
			{
				if(itr-itl>1||nxtp-p>1)return 2;
				ANS=itl->second+p->second;
				if(++cnt==2)return 2;
			}
		}
		if(cnt==0)return 0;
