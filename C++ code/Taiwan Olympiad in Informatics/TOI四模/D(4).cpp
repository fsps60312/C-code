...
void BuildPath(const int u,const int pre_ei,vector<int>&path,vector<int>&len,vector<int>&sum)
{
	path.push_back(u);
	sum.push_back(sum.back()+P[u]);
	for(const int ei:ET[u])if(ei!=pre_ei)
	{
		const Edge &e=EDGE[ei];
		const int nxt=(u==e.a?e.b:e.a);
		len.push_back(e.dist);
		BuildPath(nxt,ei,path,len,sum);
		break;
	}
}
#include<utility>
void Solve2()
{
	vector<int>path,len,sum;
	sum.push_back(0);
	for(int i=0;i<N;i++)if((int)ET[i].size()<=1)BuildPath(i,-1,path,len,sum);
	static pair<int,int>dp[30];
	for(int i=0;i<N;i++)
	{
		dp[i]=make_pair(0,0);
		if(sum[i+1]>=L)dp[i]=make_pair(1,0);
		for(int j=0;j<i&&sum[i+1]-sum[j+1]>=L;j++)
		{
			getmax(dp[i],make_pair(dp[j].first+1,dp[j].second-len[j]));
		}
	}
	const pair<int,int>&ans=dp[N-1];
	if(ans.first==0)puts("-1");
	else printf("%d %d\n",ans.first,-ans.second);
}
bool IsPathGraph()
{
	for(int i=0;i<N;i++)if((int)ET[i].size()>2)return false;
	return true;
}
int main()
{
