...
int main()
{
	freopen("tai20_10_1.txt","r",stdin);
...
//		const unsigned int i=Rand()%(unsigned int)(W-1);
		...
		for(int changed=true;changed;)
		{
			...
			for(int i=0;i<W;i++)for(int j=i+1;j<W;j++)ps.push_back(make_pair(i,j));
			...
			ps.clear();
			for(int i=0;i<W;i++)for(int j=i+2;j<W;j++)ps.push_back(make_pair(i,j));
			random_shuffle(ps.begin(),ps.end());
			for(const auto &p:ps)
			{
				int i=p.first,j=p.second;
