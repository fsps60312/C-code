...
//		const unsigned int i=Rand()%(unsigned int)(W-1);
		...
		{
			changed=false;
//			for(int tick=100;tick>=0;tick--)
			for(int i=0;i<W;i++)for(int j=i+1;j<W;j++)//for(int k=0;k<W;k++)for(int l=k+1;l<W;l++) 
			{
//				int i=Rand()%W,j=Rand()%W,k=Rand()%W,l=Rand()%W;
				swap(s[i],s[j]);//,swap(s[k],s[l]);
				if(vis.find(s)!=vis.end())swap(s[i],s[j]);//,swap(s[i],s[j]);
				else
				{
					vis.insert(s);
					if(getmin(best,Cost(s)))Print(s),changed=true;//,getmax(tick,50);
					else swap(s[i],s[j]);//,swap(s[i],s[j]);
				}
			}
