...
//{
//	return make_pair(1.0-v.first,1.0-v.second);
//}
Dp DP2;
pair<double,double>GetDP2(int a,int b,int step)
{
	...
	if(step<D)
	{
		...
		for(int d=0;d<B;d++)
		{
			pair<double,double>ta=make_pair(-2.0,-1.0);
			for(int i=0;i<D;i++)if(Get(a,i)==B)
			{
				int nxta=a;
				Set(nxta,i,d);
				getmax(ta,GetDP2(nxta,b,step+1));
			}
			assert(ta.first!=-2.0);
	...
	else
	{
		...
		for(int d=0;d<B;d++)
		{
			...
			for(int i=0;i<D;i++)if(Get(b,i)==B)
			{
				int nxtb=b;
				Set(nxtb,i,d);
				getmin(ta,GetDP2(a,nxtb,step+1));
			}
			assert(ta.first!=2.0);
			ans.first+=ta.first,ans.second+=ta.second;
		}
	...
	if(step%2==0)
	{
		...
		for(int d=0;d<B;d++)
		{
			...
			for(int i=0;i<D;i++)if(Get(a,i)==B)
			{
				int nxta=a;
				Set(nxta,i,d);
				getmax(ta,GetDP1(nxta,b,step+1));
			}
			assert(ta.first!=-2.0);
			ans.first+=ta.first,ans.second+=ta.second;
		}
	...
	else
	{
		...
		for(int d=0;d<B;d++)
		{
			...
			for(int i=0;i<D;i++)if(Get(b,i)==B)
			{
				int nxtb=b;
				Set(nxtb,i,d);
				getmin(ta,GetDP1(a,nxtb,step+1));
			}
			assert(ta.first!=2.0);
			ans.first+=ta.first,ans.second+=ta.second;
		}
...
//	const auto &p=GetDP2(F(),F(),0);
	return make_pair(GetDP1(F(),F(),0).first,GetDP2(F(),F(),0).first);
}
#include<map>
