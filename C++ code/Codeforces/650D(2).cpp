...
bool InSequence(const int a,const int b)
{
	int l=0,r=MAX_LEN;
	while(l<=r)
	{
...
int Solve(const int a,const int b)
{
	...
	{
		auto itl=ELE[len-1].lower_bound(a),itr=ELE[len+1].upper_bound(a);
		itl--;
		return S[*itl]<b&&b<S[*itr]?MAX_LEN:MAX_LEN-1;
	}
	else return InSequence(a,b)?MAX_LEN:MAX_LEN+1;
}
int main()
{
