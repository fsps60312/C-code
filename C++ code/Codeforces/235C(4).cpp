...
struct SuffixAutomaton
{
	...
	int Split(int cur,const int c)
	{
		...
		if(EDGE[c][cur]==pg)EDGE[c][cur]=wish;
	...
	void Build(const char *str)
	{
		...
		for(int i=0;i<N;i++)printf("GREEN[%d]=%d\n",i,GREEN[i]);
		TREE.Clear(N);
		for(int i=1;i<N;i++)TREE.ET[GREEN[i]].push_back(i);
		TREE.Build();
		for(const int u:vis_nodes)TREE.AddTag(u);
//		for(const int u:vis_nodes)printf("%d\n",TREE.CountTags(u));
	}
	int GetNxt(const int u,const int c)
	{
		if(!u||EDGE[c][u])return u;
		return EDGE[c][u]=EDGE[c][GetNxt(GREEN[u],c)];
	}
...
//		printf("all=%d\n",TREE.CountTags(0));
		for(int i=0,u=0,len=0;str[i];i++)
		{
			const int c=str[i]-'a';
//			u=GetNxt(u,c);
//			len=min(len+1,MAX_LEN[u]);
			while(u&&!EDGE[c][u])u=EDGE[c][u],len=MAX_LEN[u];
			if(EDGE[c][u])len++,u=EDGE[c][u];
			printf("len=%d,MAX_LEN=%d,string=",len,MAX_LEN[u]);
			for(int j=i-len+1;j<=i;j++)putchar(str[j]);
			putchar(',');
			printf("count=%d\n",TREE.CountTags(u));
			if(len>=n)
			{
				vis_nodes.push_back(u);
//				while(MAX_LEN[GREEN[u]]>=n)u=GREEN[u];
			}
		}
	...
	void Print(const int u,vector<char>now)
	{
		for(const char c:now)putchar(c);puts("");
		for(int i=0;i<26;i++)if(EDGE[i][u])
		{
			now.push_back('a'+i);
			Print(EDGE[i][u],now);
			now.pop_back();
...
//	SAM.Build("aabaaaab");
//	SAM.Print(0,vector<char>());return 0;
	while(scanf("%s",S)==1)
	{
		SAM.Build(S);
		SAM.Print(0,vector<char>());
		scanf("%d",&N);
		for(int i=0;i<N;i++)
		{
			scanf("%s",S);
