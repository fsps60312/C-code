...
//	cout("10\n");
	for(int testcase=1;testcase<=1000;testcase++)
	{
		...
		if(testcase>1)cout("\n");
		int N=Rand(1,5),M=0;
		static bool EDGE[500][500];
		for(int i=0;i<N;i++)for(int j=i+1;j<N;j++)EDGE[i][j]=(Rand(0,3)==1);
		for(int i=0;i<N;i++)for(int j=i+1;j<N;j++)if(EDGE[i][j])M++;
		cout("%d %d\n",N,M);
		for(int i=0;i<N;i++)for(int j=i+1;j<N;j++)if(EDGE[i][j])cout("%d %d\n",i+1,j+1);
	}
	return 0;
}
