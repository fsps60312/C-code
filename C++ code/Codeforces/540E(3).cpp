...
//			printf("[%d]=%d,ans=%lld\n",p.first,p.second,ans);
		...
		for(auto p=S.rbegin();p!=S.rend();++p)
		{
			...
			{
				ans+=((p->second-1)-(p->first+1))+1;
				const int r=upper_bound(locs.begin(),locs.end(),p->second-1)-locs.begin()-1;
				const int l=lower_bound(locs.begin(),locs.end(),p->first+1)-locs.begin();
				ans-=BIT2.Query(r)-BIT2.Query(l-1);
			}
			BIT1.Add(Id(V,p->second));
			BIT2.Add(Id(locs,p->first));
