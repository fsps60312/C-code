...
{
	static int a_base[2][40000];
	static Complex ra_base[2][40000];
	for(int i=0;i<N;i++)a_base[1][i]=i;
	int depth=0,n=N,oi=1;
	for(depth=0;;depth^=1,n>>=1,oi<<=1)
	{
		int *s=a_base[depth^1],*a=a_base[depth&1];
		Complex *ans=(n==N?_ans:ra_base[depth^1]);
