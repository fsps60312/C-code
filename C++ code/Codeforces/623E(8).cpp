...
vector<int>Mul(vector<int>a,vector<int>b)
{
	...
	for(const auto &v:a)ca.push_back(Complex((long double)v,0.0));
	for(const auto &v:b)cb.push_back(Complex((long double)v,0.0));
	const vector<Complex>cs=Mul(ca,cb);
	vector<int>s;
	for(const Complex &v:cs)s.push_back((int)(fmod(v.a,(long double)MOD)+0.5));//,printf("v=(%.9f,%.9f)\n",(double)v.a,(double)v.b);//,assert(fabs(v.b)<EPS);
	return s;
}
LL Mod(const LL v)
{
