...
//			puts("b");
			...
			SkipBlanks(cur);
			int c1=cur;
			if(IsDigitOrLetter(S[c1])&&ReadName(c1)==-3)
			{
				cur=c1;
				SkipOneStatement(cur);
			}
...
//			assert(name.size()==2);
			...
			{
				case'+':f.AddValue(name,parameters),assert(S[cur]==S[cur+1]),cur+=2;return true;
				case'-':f.SubValue(name,parameters),assert(S[cur]==S[cur+1]),cur+=2;return true;
				case'(':
				{
