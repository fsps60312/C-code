...
struct SuffixAutomaton
{
	...
	void Insert(const char *str)
	{
		...
		for(int repeat=0;repeat<2;repeat++)for(int i=0;str[i];i++)
		{
			const int c=str[i]-'a';
			if(!EDGE[c][last])
			{
	...
	void Match(const char *str)
	{
		...
		for(int i=0;str[i];i++)
		{
			...
			u=EDGE[c][u];
			TRACE.push_back(make_pair(MAX_LEN[u],u));
		}
	...
	void PrintAnswers()
	{
		const int n=VIS_NODES.size();
		vector<pair<int,int> >order;
		for(int i=0;i<n;i++)order.push_back(make_pair(VIS_NODES[i].size(),i));
		sort(order.begin(),order.end());
		int *ans=new int[n],i_trace=(int)TRACE.size()-1;
		for(int i=(int)order.size()-1;i>=0;i--)
		{
			const pair<int,int>&p=order[i];
			const vector<int>&vis_nodes=VIS_NODES[p.second];
			while(i_trace>=0&&TRACE[i_trace].first>=p.first)TREE.AddSize(TRACE[i_trace--].second);
			ans[p.second]=Query(p.second);
		}
