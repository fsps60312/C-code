...
bool EDGE[400][400];
bool VIS[400][400];
bool CanGo(const int a,const int b,const bool v)
{
	if(a!=b)return EDGE[a][b]==v;
	return a==N-1;
}
int main()
{
//	freopen("in.txt","r",stdin);
	while(scanf("%d%d",&N,&M)==2)
	{
		...
		while(!q.empty())
		{
			...
			for(int bnxt=0;bnxt<N;bnxt++)if(CanGo(p.second,bnxt,true)&&!VIS[p.first][bnxt])
			{
				VIS[p.first][bnxt]=true;
				for(int anxt=0;anxt<N;anxt++)if(CanGo(p.first,anxt,false))
				{
