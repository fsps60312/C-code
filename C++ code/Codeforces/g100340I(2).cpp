...
{
	int v;
	int i(){return v/10000-5000;}
	int l(){return v%10000-5000;}
	Dp(){}
	Dp(int _i,int _j,int _l):v((_i+5000)*10000+(_l+5000)){}
}DP1[2][3001],DP2[3001][3001];
...
//		printf("%s\n%s\n",SA1.s,SA2.s);
		...
		for(int i=N-1,d=0;i>=0;i--,d^=1)for(int j=M-1;j>=0;j--)
		{
			DP2[i][j]=max(Dp(i,j,SA2.Query(j,M+1+i)),i==N-1?Dp(0,0,-1):DP2[i+1][j],j==M-1?Dp(0,0,-1):DP2[i][j+1]);
		}
		for(int i=N-1,d=0;i>=0;i--,d^=1)for(int j=M-1;j>=0;j--)
		{
			DP1[d][j]=max(Dp(i,j,SA1.Query(i,N+1+j)),i==N-1?Dp(0,0,-1):DP1[d^1][j],j==M-1?Dp(0,0,-1):DP1[d][j+1]);
			if(i>=1&&j>=1)
			{
				Dp a=DP2[N-1-(i-1)][M-1-(j-1)],b=DP1[d][j];
				if(a.l()+b.l()>ans1.l()+ans2.l())
				{
					ans1=a,ans2=b;
				}
			}
			if(DP2[i][j].l()>ans1.l()+ans2.l())ans1=DP2[i][j],ans2=Dp(0,0,0);
		}
