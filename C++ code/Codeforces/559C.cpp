...
{
	if(dy<dx)return Solve(dx,dy);
//	printf("Solve(%lld,%lld)",dy,dx);
	LL ans=1LL;
	for(LL i=1LL;i<=dx;i++)Mul(ans,dy+i),Mul(ans,Inv(i));//,printf("i=%lld\n",i);
//	printf("=%lld\n",ans);
	return ans;
}
int H,W,N;
...
//	freopen("in.txt","r",stdin);
	while(scanf("%d%d%d",&H,&W,&N)==3)
	{
		...
		for(int i=0;i<N;i++)(ans-=Solve(P[i].r-1,P[i].c-1)*GetDP(i))%=MOD;//,printf("%lld*%lld\n",Solve(P[i].r-1,P[i].c-1),GetDP(i));
		printf("%I64d\n",(ans%MOD+MOD)%MOD);
	}
	return 0;
}
