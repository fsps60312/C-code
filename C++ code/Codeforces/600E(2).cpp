...
void Merge(map<int,LL>*&asum,map<int,int>*&acnt,map<int,LL>*&bsum,map<int,int>*&bcnt)
{
...
void Dfs(const int u,const int parent,map<int,LL>*&sum,map<int,int>*&cnt)
{
	(*sum)[1]=C[u],(*cnt)[C[u]]=1;
	for(const int nxt:ET[u])if(nxt!=parent)
	{
		map<int,LL>*chsum=new map<int,LL>();
		map<int,int>*chcnt=new map<int,int>();
...
//	freopen("in.txt","r",stdin);
	while(scanf("%d",&N)==1)
	{
		...
		{map<int,LL>*a=new map<int,LL>();map<int,int>*b=new map<int,int>();Dfs(0,-1,a,b);delete a;delete b;}
		for(int i=0;i<N;i++)
		{
			if(i)putchar(' ');
			printf("%lld",ANS[i]);
