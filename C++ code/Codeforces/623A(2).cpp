...
bool Solve()
{
	...
	for(int i=gid-1;i>=0&&(int)GROUP[order[i]].size()==1;i--)
	{
		COLOR[GROUP[order[i]][0]]=1;
		order.pop_back();
	}
	for(const int g:order)
	{
		for(int i=0;i<(int)GROUP[g].size();i++)
		{
			for(int j=i+1;j<(int)GROUP[g].size();j++)
			{
				if(EDGE[GROUP[g][i]][GROUP[g][j]]&&COLOR[GROUP[g][i]]!=COLOR[GROUP[g][j]])return false;
			}
		}
	}
	for(int i=0;i<(int)order.size();i++)
	{
		...
		{
			const int ga=order[i],gb=order[j];
			bool same=false,dif=false;
			for(const int u:GROUP[ga])
			{
				for(const int nxt:GROUP[gb])
				{
					assert(EDGE[u][nxt]);
					if(COLOR[u]==COLOR[nxt])same=true;
					else dif=true;
				}
			}
			if(same&&dif)return false;
			if(dif)for(const int u:GROUP[gb])COLOR[u]^=2;
		}
	}
	return true;
}
...
//	freopen("in.txt","r",stdin);
	while(scanf("%d%d",&N,&M)==2)
	{
		...
		else
		{
			...
			{
				if(EDGE[i][j])assert(abs(COLOR[i]-COLOR[j])<=1);
				else assert(abs(COLOR[i]-COLOR[j])==2);
			}
		}
	}
	return 0;
