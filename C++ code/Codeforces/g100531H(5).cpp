...
//	freopen("in.txt","r",stdin);
    ...
    while(scanf("%d",&N)==1)
    {
        ...
        A=B=6000;
        ID[RI[6000]=Point(0,0,2147483647)]=6000;
        for(const Triangle &t:S)if(t.Contains(A0)){A=ID[min(min(t.A,t.B),min(t.C,RI[A]))];}
        assert(A<6000);
        for(const Triangle &t:S)if(t.Contains(B0)){B=ID[min(min(t.A,t.B),min(t.C,RI[B]))];}
        assert(B<6000);
...
//      printf("r=%d\n",r);
        ...
        for(const Point &p:ANS)
        {
			...
			{
				const int sz=pre[p];
				while((int)ans.size()>sz)ans.pop_back();
				assert(ans.back()==p);
			}
		}
        printf("%d\n",(int)ans.size());
