...
#include<vector>
using namespace std;
const int INF=2147483647,MAX_COORDINATE=150;//1000000000;
...
int main()
{
	...
	while(scanf("%d%d",&N,&M)==2)
	{
		...
		for(int i=0,p,b,stopped=0;i<M;i++)
		{
			scanf("%d%d",&p,&b);
			if(stopped)continue;
			int result=Query(p);
			if(result==INF)continue;//stopped=1;
			else
			{
				frogs[result].tongue+=b;
				frogs[result].eaten++;
				Modify(frogs[result],result);
			}
		}
