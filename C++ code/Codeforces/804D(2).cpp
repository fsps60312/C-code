...
{
	if(a==b)return -1;
	if(DIAM[a].size()>DIAM[b].size())return Solve(b,a);
	{
	...
	for(int i=0;i<(int)DIAM[a].size();i++)
	{
		int l=0,r=(int)DIAM[b].size();
		while(l<r)
		{
			const int mid=(l+r)/2;
			if(DIAM[a][i]+1+DIAM[b][mid]<defaultDiameter)l=mid+1;
			else r=mid;
		}
		ans+=(double)(DIAM[a][i]+1)*((int)DIAM[b].size()-r)+DSUM[b][r];
		ans+=(double)defaultDiameter*r;
	}
