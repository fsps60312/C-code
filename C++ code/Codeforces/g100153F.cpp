...
map<string,map<int,bool> >DDD;
bool Ddd(const Function &f,const int parameters);
bool ExecuteSucceed(const Function &f,int cur,int &parameters)
{
...
//			puts("b");
			if(value&&!ExecuteSucceed(f,cur,parameters))return false;
			SkipOneStatement(cur);
			SkipBlanks(cur);
			int c1=cur;
			if(IsDigitOrLetter(S[c1])&&ReadName(c1)=="else")
...
//		printf("%s\n*************************\n",S.c_str()+cur);
		...
		{
			if(!ExecuteSucceed(f,cur,parameters))return false;
			SkipOneStatement(cur);
		}
		return true;
	}
...
bool Ddd(const Function &f,int parameters)
{
...
//	}
//	puts(")");
	if(DDD[f.NAME].find(parameters)!=DDD[f.NAME].end())return DDD[f.NAME][parameters];
	return DDD[f.NAME][parameters]=!ExecuteSucceed(f,f.LOC,parameters);
}
bool DfsVeryMuch()
{
	int cur=0;
	...
	for(const auto &it:FUNCTIONS)
	{
		...
		for(int p=0;p<(1<<f.PARAMETERS.size());p++)if(Ddd(f,p))return true;
	}
	return false;
}
int main()
