...
//	printf("n=%d,omega=(%.9f,%.9f)\n",n,(double)omega.a,(double)omega.b);
	...
	for(int i=0;i<n/2;i++,x=x*omega)
	{
		ans[i]=left_ans[i]*x+right_ans[i];
		ans[i+n/2]=-left_ans[i]*x+right_ans[i];
	}
}
vector<Complex>Mul(const vector<Complex>&a,const vector<Complex>&b)
{
	...
	const int n=a.size();
	const Complex omega=Complex(cos(2.0*PI/n),sin(2.0*PI/n));
...
//	}
	...
	vector<Complex>s;
	DFT(ds,Complex(omega.a,-omega.b),s);
