...
map<LL,LL>LOC;
int SA[1000000],TMP[3][1000000];
void BuildSA()
{
	int *x=TMP[0],*y=TMP[1],*c=TMP[2],p=1000;
...
int HEIGHT[1000001],RANK[1000000];
int SPARSE[1000000][20];
void BuildHEIGHT()
{
	...
	for(int i=1;i<N;i++)SPARSE[i][0]=HEIGHT[i];
	for(int d=1;(1<<d)<=N;d++)
	{
		for(int i=1;i+(1<<d)<=N;i++)
		{
			SPARSE[i][d]=min(SPARSE[i][d-1],SPARSE[i+(1<<(d-1))][d-1]);
		}
	}
}
int Query(const int ia,const int ib)
{
	...
	return min(SPARSE[a][d],SPARSE[b-(1<<d)+1][d]);
}
void TestSA()
{
	char s[]="aabaaaab";
