...
struct SegTree
{
	...
	inline void PutDownADD(const int id)
	{
		if(!ADD[id])return;
		const int v=ADD[id];ADD[id]=0;
		ADD[id*2]+=v,ADD[id*2+1]+=v;
//		MN[id*2]+=v,MN[id*2+1]+=v;
	}
	inline void PutDown(const int id)
	{
		if(LOW[id]!=-INF)
		{
			PutDownADD(id*2),PutDownADD(id*2+1);
			const int v=LOW[id];LOW[id]=-INF;
			getmax(LOW[id*2],v),getmax(LOW[id*2+1],v);
			getmax(MN[id*2],v),getmax(MN[id*2+1],v);
		}
		PutDownADD(id);
	}
	void Build(const int id,const int l,const int r,const int val)
	{
		MN[id]=val,ADD[id]=0,LOW[id]=-INF;
		if(l<r)
		{
			const int mid=(l+r)/2;
	...
	{
		if(r<bl||br<l)return;
		if(bl<=l&&r<=br){/*MN[id]+=val,*/ADD[id]+=val;return;}
		const int mid=(l+r)/2;
		PutDown(id);
	...
	{
		if(r<bl||br<l)return;
		if(bl<=l&&r<=br){PutDownADD(id),getmax(MN[id],val),getmax(LOW[id],val);return;}
		const int mid=(l+r)/2;
		PutDown(id);
	...
	{
		assert(MN[id]<=0);
		if(l==r){SetLow(r,r,INF);return r;}
		const int mid=(l+r)/2;
