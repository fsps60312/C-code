...
//	for(int i=1;i<(int)queries.size();i++)assert(LOC[QUERY[queries[i-1]]]<LOC[QUERY[queries[i]]]);
	...
	const int mid=(l+r)/2;
	for(const int v:numbers)(LIFE[v]<=mid?left_numbers:right_numbers).push_back(v);
	for(const int v:queries)(v<=mid?left_queries:right_queries).push_back(v);
	if(true)
	{
