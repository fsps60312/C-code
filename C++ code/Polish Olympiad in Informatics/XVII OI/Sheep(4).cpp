		...
		for(int gap=2;gap<N;gap++)
		{
			for(int i=0;i<N;i++)
			{
				...
				if(VALID[i][j])
				{
					...
					{
						const int k=(i+_k)%N;
						if((CNT[i][j]-(CNT[i][k]+CNT[k][j]))%2==0)(ONE[i][j]+=ONE[i][k]*ONE[k][j])%=MOD;
					}
				}
			}
		}
		int ans=(VALID[1][N-1]?ONE[1][N-1]:0);
		for(int i=2;i+1<N;i++)if(VALID[0][i])(ans+=ONE[0][i]*ONE[i][0])%=MOD;//,printf("%d\n",ONE[0][i]*ONE[i][0]);
		printf("%d\n",ans);
	}
	return 0;
}
