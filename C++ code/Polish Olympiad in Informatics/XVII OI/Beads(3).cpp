...
int N,S[400000],SA[400000],RANK[400000],HEIGHT[400000],ANS[200001];
void GetRanks()
{
	static vector<int>sot[400000];
	static int last[200001];
	...
	for(int i=0;i<N*2;i++)
	{
		for(int j=0;j<(int)sot[i].size();j++)
		{
			const int gap=sot[i][j];
			if(last[gap]!=-1&&QuerySparse(last[gap]+1,i)<gap)++ANS[gap];
			last[gap]=i;
		}
