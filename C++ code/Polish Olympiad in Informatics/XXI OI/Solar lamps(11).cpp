...
inline LL GetLongLong()
{
	...
	LL v=0LL;
	while('0'<=c&&c<='9')v=v*10LL+(c-'0'),c=GetChar();
	return v*sign;
}
int N,YSZ;
struct Node
...
int CUR=0;
inline int NewNode(const int _loc,const int _sz)
{
...
inline int SumOfLch(const vector<int>&s,const int l,const int r)
{
	int ans=0;
...
inline void MoveToCh(vector<int>&s,const int d,const int l,const int r)
{
	for(int i=0;i<(int)s.size();i++)if(s[i]!=-1)
	{
		const int tag=NODE[s[i]].tag;
...
struct SegTree
{
	...
	inline void Modify(int i,const int val){++i;while(i<=YSZ)Insert(S[i],0,N-1,val,1),i+=(i&(-i));}
	inline int Query(int i,const int k)
	{
		++i;
