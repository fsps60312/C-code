...
//	\-
	...
	{
		DP[0][State(i,i+1,i+2,Combine(i==0?0:1,0,i+2==N*3?0:2))]=NULL;
		DP[0][State(i+2,i+1,i,Combine(i+2==N*3?0:2,0,i==0?0:1))]=NULL;
	}
//	--
//	/-
//	\-
...
//			---  <- id2
			...
			{
				if(loc==0)DP[column+1][State(N*3,id1,id2,Combine(1,2,1))]=&s;
				else      DP[column+1][State(id2,id1,N*3,Combine(1,2,1))]=&s;
			}
//			 *-  ->
//			---  -> id1
//			---  <- id2
			...
			{
				if(loc==0)DP[column+1][State(  1,id1,id2,Combine(2,2,1))]=&s;
				else      DP[column+1][State(id2,id1,  1,Combine(1,2,2))]=&s;
			}
		}
		else//in
		{
...
//			---  -> id2
			...
			{
				if(loc==0)DP[column+1][State(  1,id1,id2,Combine(2,1,2))]=&s;
				else      DP[column+1][State(id2,id1,  1,Combine(2,1,2))]=&s;
			}
//			 *-  <-
//			---  <- id1
//			---  -> id2
			...
			{
				if(loc==0)DP[column+1][State(N*3,id1,id2,Combine(1,1,2))]=&s;
				else      DP[column+1][State(id2,id1,N*3,Combine(2,1,1))]=&s;
			}
		}
	}
	else if(loc==1)
...
//		---  <-
		...
		else//in
		{
			...
			{
				assert(id2>id1);
				if(id1-1>=1)  DP[column+1][State(id1,id1-1,id2,Combine(0,id1-1==1?0:1,id2==N*3?0:2))]=&s;
				if(id2+1<=N*3)DP[column+1][State(id1,id2+1,id2,Combine(id1==1?0:1,id2+1==N*3?0:2,0))]=&s;
			}
		}
//		---  ->
//		 *-  <>
//		---  <-
		if(d==2)//out
		{
			...
			{
				assert(id2>1);
				DP[column+1][State(id1,N*3,id2,Combine(2,1,1))]=&s;
			}
			if(id2-1>=3&&(id2-1)%2==1&&id1-id2>=4&&(id1-id2)%2==0)
			{
				assert(id1<N*3);
				DP[column+1][State(id1,1,id2,Combine(2,2,1))]=&s;
			}
		}
//		---  <-
//		 *-  <>
//		---  ->
		else
		{
			...
			{
				assert(id2<N*3);
				DP[column+1][State(id1,1,id2,Combine(1,2,2))]=&s;
			}
			if(N*3-id2>=3&&(N*3-id2)%2==1&&id2-id1>=4&&(id2-id1)%2==0)
			{
				assert(id1>1);
				DP[column+1][State(id1,N*3,id2,Combine(1,1,2))]=&s;
			}
		}
	}
	else assert(0);
...
void UpdateFromThreeWay(const int column,const State &s)
{
	...
	{
		assert(id1+dis[1]!=id0&&id2+dis[2]!=id0&&id2+dis[2]!=id1);
...
void Print(const State &s,const int column)
{
	...
	for(int j=0;j<3;j++)
	{
		for(int i=0;i<=column;i++)printf(" %d",ans[i][j]);
		puts("");
	}
	puts("");
	assert(cur==NULL);
