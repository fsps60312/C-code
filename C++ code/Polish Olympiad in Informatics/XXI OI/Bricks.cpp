...
int ANS[999998];
void Fill(const vector<Brick>&S,const vector<int>&order,const int exclude1,const int exclude2)
{
	vector<int>s;
	for(int i=0;i<K;i++)if(i!=exclude1&&i!=exclude2)
	{
...
bool Solve(const vector<Brick>&S)
{
	...
	if(N&1)
	{
		...
		else if(S[0].cnt==N/2+1)
		{
			...
			for(int i=1;i<K;i++)
			{
				for(int j=0;j<S[i].cnt;j++)ANS[cur]=S[i].color,cur+=2;
			}
			assert(cur==N-2);
			return true;
		}
		else
		{
			...
			{
				assert(S[a].cnt>=2);
				for(int i=0;i<K;i++)if(i!=a&&S[i].cnt==N/2)
				{
					++b;
					while(b<K&&S[b].color==Q)b++;
					if(b==K)return false;
					goto another_way;
				}
			...
			else
			{
				...
				{
					for(int i=0;i<K;i++)if(i!=a&&i!=b&&S[i].cnt==N/2)return false;
				...
				{
					const int exceed=(N/2+1)-S[a].cnt-S[b].cnt;
					for(int i=0;i<S[a].cnt;i++)ANS[i*2]=S[a].color;
...
//	freopen("in.txt","r",stdin);
	while(scanf("%d%d%d",&K,&P,&Q)==3)
	{
		...
		if(P==Q&&S[P-1].cnt==1)goto no_solution;
