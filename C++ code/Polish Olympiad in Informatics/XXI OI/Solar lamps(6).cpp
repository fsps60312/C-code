...
void Insert(int &id,const int l,const int r,const int loc)
{
	assert(l<=loc&&loc<=r);
...
int SumOfLch(const vector<int>&s)
{
	int ans=0;
	for(int i=0;i<(int)s.size();i++)if(s[i]!=-1&&NODE[s[i]].l!=-1)ans+=NODE[NODE[s[i]].l].sz;
	return ans;
}
void MoveToCh(vector<int>&s,const int d)
{
	for(int i=0;i<(int)s.size();i++)if(s[i]!=-1)s[i]=(d==0?NODE[s[i]].l:NODE[s[i]].r);
}
