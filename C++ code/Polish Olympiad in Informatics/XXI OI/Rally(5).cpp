...
//	freopen("in.txt","r",stdin);
	while(scanf("%d%d",&N,&M)==2)
	{
		...
		int uppoints=0;
		static PriorityQueue midedges,downpoints;
		midedges.Clear(N+2+1);
		downpoints.Clear(N+2+1);
		for(int i=0;i<N+2;i++)downpoints.Push(DT[i]);
		...
		for(int _u=0;_u<N+2;_u++)
		{
			...
			downpoints.Erase(DT[u]);
			if(u<N)
			{
				const int ta=max(midedges.Top(),max(uppoints,downpoints.Top()));
		...
		printf("%d %d\n",ans_u+1,ans_dist-1);
		assert(midedges.S.empty()),assert(downpoints.S.empty());
	}
	return 0;
}
