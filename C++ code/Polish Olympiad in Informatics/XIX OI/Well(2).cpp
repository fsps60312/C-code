...
int N;
LL M;
inline pair<LL,int>CutRight(int x,int h,const int slope,const int *hs)
{
	LL sum=0LL;
	for(;x<N&&h<hs[x];x++,h+=slope)sum+=hs[x]-h;
	return make_pair(sum,x);
}
inline pair<LL,int>CutLeft(int x,int h,const int slope,const int *hs)
{
	LL sum=0LL;
...
int *HS;
struct CmpHS{bool operator()(const int a,const int b)const{return HS[a]!=HS[b]?HS[a]<HS[b]:a<b;};};
int Solve(const int slope,const int *_hs)
{
	...
	while(!pq.empty())
	{
		const int x=*pq.begin();pq.erase(pq.begin());
