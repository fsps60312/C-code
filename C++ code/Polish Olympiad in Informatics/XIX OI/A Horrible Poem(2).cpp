...
struct SuffixArray
{
	...
	{
		assert(a<=b);
		for(int bit=0;;bit++)if((1<<(bit+1))>(b-a+1))
		{
	...
	int QueryLCS(const int _a,const int _b)
	{
		...
		if(a>b)swap(a,b);
		assert(a<b);
//		printf("LCS(%d,%d)(%d,%d)=%d\n",_a,_b,a,b,QuerySparse(a+1,b));
...
{
//	SA.Build("aabaaaab",8);
//	for(int i=0;i<8;i++)printf("%d ",SA.SA[i]);puts("");
//	for(int i=0;i<8;i++)printf("%d ",SA.HEIGHT[i]);puts("");
//	freopen("in.txt","r",stdin);
	...
	for(int a,b;querycount--;)
	{
		...
		int ans=len;
		for(int i=0;i+1<(int)fs.size();i++)if(SA.QueryLCS(a,a+fs[i])>=len-fs[i])
		{
			ans=fs[i];break;
		}
		printf("%d\n",ans);
