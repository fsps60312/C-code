...
//	freopen("pen3a.in","r",stdin);
	...
	vector<int>trees;
	for(int i=1;i<=N;i++)if(SALARY[i]!=0&&FindZero(i)>=1)trees.push_back(i);
	sort(trees.begin(),trees.end(),CmpSalary);
	vector<int>stk;
	for(int i=1;i<=N;i++)if(!vis[i])stk.push_back(i);
	while(!trees.empty())
	{
		const int u=trees.back(),bound=(trees.size()==1?0:trees[trees.size()-2]);
