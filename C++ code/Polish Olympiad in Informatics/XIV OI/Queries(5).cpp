...
unsigned int LBound(const vector<unsigned int>&s,const unsigned int val){return (unsigned int)(lower_bound(s.begin(),s.end(),val)-s.begin());}
unsigned int LBA[50002],LBS[50002];
int main()
{
//	freopen("in.txt","r",stdin);
	BuildPrimes();
	BuildADDSUB();
	for(int n=1;n<=50001;n++)LBA[n]=LBound(ADD,n),LBS[n]=LBound(SUB,n);
	...
	while(testcount--)
	{
		...
		for(unsigned int d=1,pre_d;d<=a&&d<=b;)
		{
			...
			ans+=(unsigned int)(LBA[d]-LBA[pre_d])*((a/pre_d)*(b/pre_d));
			ans-=(unsigned int)(LBS[d]-LBS[pre_d])*((a/pre_d)*(b/pre_d));
		}
		printf("%u\n",ans);
	}
	return 0;
