#include<cstdio>
#include<cassert>
#include<vector>
#include<utility>
#include<set>
#include<algorithm>
using namespace std;
int G[1002][1002];
bool WANT[1002][1002];
struct DisjointSets
{
	pair<int,int>S[1002][1002];
	void Clear(const int N,const int M)
	{
		for(int i=1;i<=N;i++)for(int j=1;j<=M;j++)S[i][j]=make_pair(i,j);
	}
	pair<int,int>Find(const pair<int,int>&a){return S[a.first][a.second]==a?a:(S[a.first][a.second]=Find(S[a.first][a.second]));}
	void Merge(pair<int,int>a,pair<int,int>b)
	{
		a=Find(a),b=Find(b);
		assert(a!=b);
//		if(!WANT[b.first][b.second]||(WANT[a.first][a.second]&&G[a.first][a.second]<G[b.first][b.second]))swap(a,b);
		S[a.first][a.second]=b;
	}
}DJ;
int N,M;
vector<pair<int,int> >H[1001];
bool VIS[1002][1002];
int main()
{
//	freopen("in.txt","r",stdin);
	while(scanf("%d%d",&N,&M)==2)
	{
		for(int i=0;i<=1000;i++)H[i].clear();
		for(int i=1;i<=N;i++)for(int j=1;j<=M;j++)
		{
			scanf("%d",&G[i][j]);
			WANT[i][j]=(G[i][j]>=1);
			if(G[i][j]<0)G[i][j]*=-1;
			H[G[i][j]].push_back(make_pair(i,j));
		}
		for(int i=0;i<=N+1;i++)for(int j=0;j<=M+1;j++)VIS[i][j]=false;
		DJ.Clear(N,M);
		int ans=0;
		for(int h=0;h<=1000;h++)
		{
			for(int i=0;i<(int)H[h].size();i++)
			{
				const pair<int,int>&p=H[h][i];
				static int d[4][2]={{-1,0},{0,-1},{0,1},{1,0}};
				vector<pair<int,int> >ch;
				for(int j=0;j<4;j++)
				{
					const pair<int,int>&nxtp=make_pair(p.first+d[j][0],p.second+d[j][1]);
					if(VIS[nxtp.first][nxtp.second])ch.push_back(DJ.Find(nxtp));
				}
				sort(ch.begin(),ch.end()),ch.resize(unique(ch.begin(),ch.end())-ch.begin());
				assert(!VIS[p.first][p.second]&&DJ.Find(p)==p);
				VIS[p.first][p.second]=true;
				bool found=false;
				for(int j=0;j<(int)ch.size();j++)if(WANT[ch[j].first][ch[j].second]&&G[ch[j].first][ch[j].second]<h)
				{
					for(int k=0;k<(int)ch.size();k++)if(k!=j)
					{
						if(WANT[ch[k].first][ch[k].second]&&G[ch[k].first][ch[k].second]==h)--ans;
						DJ.Merge(ch[k],ch[j]);
					}
					DJ.Merge(p,ch[j]);
					found=true;break;
				}
				if(found)continue;
				for(int j=0;j<(int)ch.size();j++)if(WANT[ch[j].first][ch[j].second])
				{
					for(int k=0;k<(int)ch.size();k++)if(k!=j)
					{
						if(WANT[ch[k].first][ch[k].second]&&G[ch[k].first][ch[k].second]==h)--ans;
						DJ.Merge(ch[k],ch[j]);
					}
					DJ.Merge(p,ch[j]);
					found=true;break;
				}
				if(found)continue;
				if(WANT[p.first][p.second])++ans;
				for(int j=0;j<(int)ch.size();j++)DJ.Merge(ch[j],p);
			}
		}
		printf("%d\n",ans);
	}
	return 0;
}
