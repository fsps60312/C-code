...
//		printf("%d %d %d\n",N,W_UPON,W_DOWN);
	...
	inline int GetCost(const QueryType &q)const{return (W_UPON*V[q.UPON]+W_DOWN*V[q.DOWN])-2*max(0LL,V[q.X]-V[q.UPON],V[q.DOWN]-V[q.X]);}
...
//		printf("id=%d\n",q.ID);
		...
		const int cost=2*max(0LL,V[q.X]-V[q.UPON],V[q.DOWN]-V[q.X]);
		{
