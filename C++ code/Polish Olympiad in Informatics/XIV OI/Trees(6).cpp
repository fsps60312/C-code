...
struct SegTree
{
	...
	{
		D[id]=(IS_MX?-INF:INF);
		if(l==r){S[r].clear(),S[r].insert(D[id]);return;}
		const int mid=(l+r)/2;
	...
	void Insert(const int id,const int l,const int r,const int loc,const int val)
	{
		if(l==r)
		{
			assert(r==loc);
			S[r].insert(val);
			D[id]=(IS_MX?*S[r].rbegin():*S[r].begin());
			return;
		}
		const int mid=(l+r)/2;
	...
	void Erase(const int id,const int l,const int r,const int loc,const int val)
	{
		...
		{
			assert(r==loc);
			multiset<int>::iterator it=S[r].find(val);
			assert(it!=S[r].end());
			S[r].erase(it);
			D[id]=(IS_MX?*S[r].rbegin():*S[r].begin());
			return;
	...
	int Query(const int bl,const int br)const{assert(bl<=br);return Query(1,0,N-1,bl,br);}
};
vector<int>V;
struct LargeSegTree
{
...
//		printf("%lld %lld %lld\n",N,W_UPON,W_DOWN);
	...
	inline int GetCost(const QueryType &q)const{return (W_UPON*V[q.UPON]+W_DOWN*V[q.DOWN])-max(0LL,V[q.X]-V[q.UPON],V[q.DOWN]-V[q.X]);}
...
//		printf("id=%d\n",q.ID);
		...
		if(ta!=INF)getmin(ANS[q.ID],2*(ta-max(0LL,V[q.X]-V[q.UPON],V[q.DOWN]-V[q.X])));
//		assert(0);
