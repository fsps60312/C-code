...
inline bool operator<(const QueryType &a,const QueryType &b)
{
...
{
	int N,SZ;
	multiset<int>S[50000];
	int D[200000];
	inline void Build(const int _N)
	{
		N=_N,SZ=0;
	...
	void Erase(const int id,const int l,const int r,const int loc,const int val)
	{
		if(l==r)
		{
			S[r].erase(S[r].find(val));
			D[id]=(S[r].empty()?INF:*S[r].begin());
			return;
		}
		const int mid=(l+r)/2;
	...
	inline void Insert(const int loc,const int val){Insert(1,0,N-1,loc,val),++SZ;}
	inline void Erase(const int loc,const int val){Erase(1,0,N-1,loc,val),--SZ;}
	...
	inline int Query(const int bl,const int br)const{return Query(1,0,N-1,bl,br);}
};
vector<int>V;
struct LargeSegTree
{
...
QueryType GetQueryType(const int i){return QueryType(i,H[i],max(H[i-1],H[i+1]),min(H[i-1],H[i+1]));}
void Discretize()
{
	V.clear();
...
//		printf("id=%d\n",q.ID);
		...
		if(ta!=INF)getmin(ANS[q.ID],2*(ta-max(0,V[q.X]-V[q.UPON],V[q.DOWN]-V[q.X])));
...
main()
{
//	freopen("drz1.in","r",stdin);
//	freopen("in.txt","r",stdin);
//	printf("%llu %llu\n",sizeof(int),sizeof(multiset<int>));
	while(scanf("%d",&N)==1)
	{
