...
struct SegTree
{
	...
	void Build(const int _N,const bool _IS_MX){N=_N,IS_MX=_IS_MX;Build(1,0,N-1);}
	...
	void Erase(const int loc,const int val){Erase(1,0,N-1,loc,val);}
	...
	int Query(const int bl,const int br){return Query(1,0,N-1,bl,br);}
};
struct LargeSegTree
{
	int W_UPON,W_DOWN;
	...
	inline int GetCost(const QueryType &q){return max(0,q.X-q.UPON,q.DOWN-q.X);}
	inline int GetMinXValue(const QueryType &q){return GetCost(q)+(q.X+W_UPON*q.UPON+W_DOWN*q.DOWN);}
	inline int GetMaxXValue(const QueryType &q){return -GetCost(q)+(q.X-W_UPON*q.UPON-W_DOWN*q.DOWN);}
	...
	int QueryMinX(const int bl,const int br){return MIN_X.Query(bl,br);}
	int QueryMinCost(const int bl,const int br){return MIN_COST.Query(bl,br);}
	int QueryMaxX(const int bl,const int br){return MAX_X.Query(bl,br);}
}SEG[3];
...
void Solve()
{
	...
	{
		assert((int)(m1.size()+m2.size()+m3.size())==_qi);
		...
		for(int i=0;i<3;i++)
		{
			const int adjust=(i==0?-V[q.X]:(i==1?0:V[q.X]));
			const int v1=SEG[i].QueryMinX(0,q.UPON),v2=SEG[i].QueryMinCost(q.UPON,q.DOWN),v3=SEG[i].QueryMaxX(q.DOWN,M-1);
			if(v1!=INF)getmin(ANS[q.ID],-(cost+(v1-V[q.UPON])+adjust));
			if(v2!=INF)getmin(ANS[q.ID],-(cost+v2+adjust));
			if(v3!=-INF)getmin(ANS[q.ID],-(cost+(V[q.DOWN]-v3+adjust)));
		}
//		assert(0);
