...
//	freopen("in.txt","r",stdin);
	...
	{
		S.clear();
		for(int i=1;i<=N;i++)scanf("%d",&A[i]),S.push_back(make_pair(i-A[i],i));
//		for(int i=0;i<N;i++)scanf("%d",&A[i]);
		...
		sort(S.begin(),S.end());
		int ans=0;
		for(int i=0;i<N;i++)if(S[i].first>=0)
		{
			int l=0,r=ans;
			while(l<r)
			{
				const int mid=(l+r)/2;
				if(A[S[i].second]>A[S[dp[mid]].second])l=mid+1;
				else r=mid;
			}
			dp[r]=i;
			if(r==ans)++ans;
...
//			printf("A[%d]=%d\n",reserve[i],A[reserve[i]]);
			...
			assert(removed==reserve[i]-A[reserve[i]]&&cur<=reserve[i]);
			cur=reserve[i]+1;
		}
		printf("%d\n",(int)bns.size());
