...
void RebuildGraph()
{
	...
	for(int i=0;i<N;i++)for(int j=0;j<(int)ET[i].size();j++)
	{
...
void FindWinners(vector<int>&output)
{
	...
	for(int i=0;i<SCC_CNT;i++)for(int j=0;j<(int)ET[i].size();j++)degree_in[ET[i][j]]++;
	queue<int>scc_win;
	set<int>unknown;
	...
	while(!scc_win.empty())
	{
		...
		output.push_back(u);
		for(set<int>::iterator nxt=unknown.begin();nxt!=unknown.end();)
		{
	...
	for(int i=0;i<(int)output.size();i++)is_win[output[i]]=true;
	output.clear();
