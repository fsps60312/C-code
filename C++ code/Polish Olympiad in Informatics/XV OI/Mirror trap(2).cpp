...
struct Point
{
	...
	Point(){}
	Point(const int _ID,const int _CORNER,const int _X,const int _Y,const int _D1,const int _D2):ID(_ID),CORNER(_CORNER),X(_X),Y(_Y),VIS(false),D1(_D1),D2(_D2){}
};
...
vector<Point>P;
void AddCorner(const int corner,const int x0,const int y0,const int x1,const int y1,const int x2,const int y2)
{
	const Point p=Point((int)P.size(),corner,x0,y0,Direction(x0,y0,x1,y1),Direction(x0,y0,x2,y2));
	P.push_back(p);
}
void AddWall(const int x1,const int y1,const int x2,const int y2)
{
	...
	{
		const Point p=Point((int)P.size(),-1,x0,y0,Direction(x0,y0,x1,y1),Direction(x0,y0,x2,y2));
		P.push_back(p);
	}
}
int N;
void ReadInput()
{
	...
	for(int i=0;i<N;i++)
	{
		AddCorner(i+1,x[i],y[i],x[(i-1+N)%N],y[(i-1+N)%N],x[(i+1)%N],y[(i+1)%N]);
...
int EmitDirection(const int d2,const int d1)//0¡ù1¡û2¡ú3¡ø
{
	if(d1==0&&d2==1)return 0;
	if(d1==1&&d2==2)return 1;
	if(d1==2&&d2==3)return 2;
	if(d1==3&&d2==0)return 3;
	if(d1==1&&d2==0)return 2;
	if(d1==2&&d2==1)return 3;
	if(d1==3&&d2==2)return 0;
...
//	printf("(%d,%d,%d)\n",p.X,p.Y,direction);
	if((p.D1+p.D2)%2==1)
	{
		...
		assert(d!=direction);
		if((d+2)%4==direction){assert(!p.VIS);p.VIS=true;ANS.push_back(p.ID);return;}
		else Travel(NxtPoint(p,direction),direction);
	}
	else
	{
...
//	freopen("in.txt","r",stdin);
	...
	{
		const int a1=P[ANS[i]].CORNER,a2=P[ANS[i+1]].CORNER;
		assert(a1!=-1&&a2!=-1);
		printf("%d %d\n",a1,a2);
	}
