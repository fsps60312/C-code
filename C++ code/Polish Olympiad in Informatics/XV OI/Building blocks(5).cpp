...
struct Bit
{
	...
	void Add(int i,const int val){for(++i;i<=N;i+=i&(-i))S[i]+=val;}
	int Query(int i)const{int ans=0;for(++i;i;i-=i&(-i))ans+=S[i];return ans;}
...
//	freopen("out.txt","w",stdout);
	...
	if(true)
	{
		static SegTree upon,down;
		upon.Build(N-1),down.Build(N-1);
		for(int i=0;i<N;i++)upon.Set(i,(LL)H[i]);
		LL upon_sz=N,down_sz=0LL;
		for(multimap<int,int>::iterator it=sot.begin(),iq=queries.begin();it!=sot.end()&&iq!=queries.end();++it)
		{
			...
			for(;iq!=queries.end()&&iq->first==it->first;++iq)
			{
				...
				const LL ta=(upon.QuerySum(l,r)-upon_sz*it->first)+(down_sz*it->first-down.QuerySum(l,r));
