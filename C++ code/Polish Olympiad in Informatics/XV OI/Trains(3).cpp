...
struct Sparse
{
	...
	Sparse(const vector<pair<int,int> >&_S)
	{
		...
		for(int i=0;i<N;i++)
		{
			if(i)assert(_S[i-1].first<_S[i].first);
...
int GetId(const string s)
{
	...
	assert((int)CNT.size()==sz);
	CNT.push_back(vector<pair<int,int> >(1,make_pair(0,0)));
	return ID[s]=sz;
}
int N,L,M;
...
void ChangeCnt(const int id,const int time,const int dif)
{
	...
	else
	{
		...
		assert(p.first==time);
		p.second+=dif;
		if(p.second!=CNT[id].back().second)CNT[id].push_back(p);
	}
}
int main()
{
...
//				for(int k=l;k<=r;k++)printf(" (%d,%d)%d",k,k,sparse[TRAIN[i][j].second].Query(k,k));
