...
vector<vector<LL> >DP;
LL GetDP(const int i,const int p,const vector<int>&f)
{
	while((int)DP[i].size()<=p)DP[i].push_back(DP[i].back()*f[i]%MOD);
	return DP[i][p];
}
int main()
{
...
//		for(int i=0;i<(int)factors.size();i++)printf(" %d",factors[i]);puts("");
		...
		for(int i=0;i<(int)factors.size();i++)DP[i].push_back(1);
...
//			printf("mul=%lld, SEG.S[1]=%lld\n",mul,SEG.S[1]);
			for(int j=0;j<(int)factors.size();j++)(mul*=GetDP(j,CNT[j],factors))%=MOD;
			(ans+=val*mul)%=MOD;
			Mul(val,n,factors);
