...
Cycle CheckMinCycle(const vector<Point>&ps,const int j,const int k,const double radius)
{
	Cycle ans=Cycle(ps[j],ps[k]);
	for(int i=0;i<k;i++)if(!ans.Contains(ps[i]))
	{
		ans=Cycle(ps[i],ps[j],ps[k]);
		if(ans.r>radius)return ans;
	}
	return ans;
}
Cycle CheckMinCycle(const vector<Point>&ps,const int j,const double radius)
{
	Cycle ans=Cycle(ps[0],ps[j]);
	for(int i=1;i<j;i++)if(!ans.Contains(ps[i]))
	{
		ans=CheckMinCycle(ps,j,i,radius);
		if(ans.r>radius)return ans;
	}
	return ans;
}
Cycle CheckMinCycle(const vector<Point>&ps,const double radius)
{
	Cycle ans=Cycle(ps[0],0.0);
	for(int i=1;i<(int)ps.size();i++)if(!ans.Contains(ps[i]))
	{
		ans=CheckMinCycle(ps,i,radius);
		if(ans.r>radius)return ans;
	}
	return ans;
}
Point P[100000];
Cycle CheckMinCycle(const int l,const int r,const double radius)
{
	vector<Point>ps;
	for(int i=l;i<=r;i++)ps.push_back(P[i]);
	random_shuffle(ps.begin(),ps.end());
	return CheckMinCycle(ps,radius);
}
SparseTable SP[2];
...
//		printf("%d(%d,%d),%.9f\n",i,l,r,SP[i].Query(l,r));
		const double d=SP[i].Query(l,r);
		if(d/2.0>radius)return false;
		upbound+=d*d;
	}
	if(sqrt(upbound)/2.0<=radius)return true;
	return CheckMinCycle(l,r,radius).r<=radius;
}
int N,M;
void BuildSP()
{
...
//	SP[1].Build(N,s);
	for(int i=0;i<N;i++)s[i]=(P[i].x+P[i].y)/SQRT2;
	SP[0].Build(N,s);
	for(int i=0;i<N;i++)s[i]=(P[i].x-P[i].y)/SQRT2;
	SP[1].Build(N,s);
}
...
bool Solve(const double radius)
{
	...
	for(int i=0;i<N;)
	{
		...
		ANS.push_back(CheckMinCycle(i,r,radius).o);
		i=r+1;
	}
	return true;
}
