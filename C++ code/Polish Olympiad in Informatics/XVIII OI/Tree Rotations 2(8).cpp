...
//		puts("pass");
		...
		const int lsz=lch->sz,rsz=rch->sz;
		LL ta=0LL;
		for(int i=0;i<lsz;++i)ta+=(upper_bound(s2.begin(),s2.end(),s1[i])-s2.begin());
		ANS+=min(ta,((lch->sz)*(rch->sz))-ta);
		for(int i=0,j=0;i<lsz||j<rsz;)
		{
			if(j==rsz||(i<lsz&&s1[i]<s2[j]))s0.push_back(s1[i]),++i;
			else s0.push_back(s2[j]),++j;
		}
//		assert(s0.size()==s1.size()+s2.size());
//		assert(0<s0[0]);
//		for(int i=1;i<sz;++i)assert(s0[i-1]<=s0[i]);
	}
};
int N;
int main()
...
//	assert(N<=5000);
	...
	Node *root=new Node();
	vector<int>s;
	if(ANS<=25*N)ANS=0LL,root->Solve2(s);
