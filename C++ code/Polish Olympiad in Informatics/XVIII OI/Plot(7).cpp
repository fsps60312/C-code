...
unsigned int Rand()
{
	static unsigned int seed=20160410;
...
//	}
	...
	unsigned int size()const{return Sz(S);}
};
...
//		printf("MOD=%d,(%d,%d),(%d,%d)\n",MOD,l,r,li,ri);
		for(int i=li;i<=ri;i++)ps.PushBack(PS[i]);//,printf("%d,",(int)ps.size());
//		printf("(%d,%d)\n",(int)ps.size(),(ri-li+1)*MOD);
		assert((int)ps.size()==(ri-li+1)*MOD);
...
//	assert((int)ps.size()==r-l+1);
	...
	else
	{
		...
		random_shuffle(vps.begin(),vps.end());
		ps.PushBack(vps);
		ans=MinCycle(ps,Cycle(ps[0],0.0),1);
	}
	assert((int)ps.size()==r-l+1);
	ps.Recover();
	return ans;
...
void BuildCYC()
{
	...
	vector<int>szs;
	for(int i=0;i<m;i++)szs.push_back(Sz(PS[i]));
	for(int i=0;i<m;i++)
	{
		Vector ps=Vector();
		ps.PushBack(PS[i]);
...
//	for(int i=0;i<m;i++)assert(Sz(PS[i])==szs[i]);
	...
	for(int i=0;i<m;i++)assert((int)Sz(PS[i])==szs[i]);
//	for(int i=0;i<m;i++)printf(" %d",(int)Sz(PS[i]));puts("");
}
vector<Point>ANS;
