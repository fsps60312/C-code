...
//			printf("u=%d, fail=%d, depth=%d, parent=%d, must/cant=(%d,%d)\n",u,FAIL[u],DEPTH[u],PARENT[u],(int)LONG_MUST_END[u],(int)LONG_CANT_END[u]);
	...
	bool MustBeEnd(const int u)
	{
		...
		for(int o=u;o;o=FAIL[o])if(REAL_END[o])nxts.push_back(o);
		for(int i=(int)nxts.size()-1,nxt=u,dis=0;i>=0;i--)
		{
			const int o=nxts[i];
			assert(dis<DEPTH[o]);
			while(dis<DEPTH[o])nxt=PARENT[nxt],dis++;
			if(MustBeEnd(nxt))goto return_true;
		}
		goto return_false;
		return_true:;
