...
int BuildSZDP(const int u,const int parent)
{
	PARENT[u]=parent;
	int &szdp=SZDP[u]=1;
	for(int cur=HEAD[u];cur!=-1;cur=NEXT[cur])if(ET[cur]!=parent)
	{
		szdp+=BuildSZDP(ET[cur],u);
	}
	return szdp;
}
int N;
...
{
	if(ans!=-1)return ans;
	if(parent==PARENT[u])return ans=SZDP[u];
	else
	{
		assert(parent!=-1);
		return ans=N-SZDP[parent];
	}
//	ans=1;
//	for(int cur=HEAD[u];cur!=-1;cur=NEXT[cur])if(ET[cur]!=parent)
//	{
