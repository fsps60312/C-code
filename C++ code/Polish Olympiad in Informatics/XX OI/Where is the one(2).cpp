...
vector<int>Merge(vector<int>*a,vector<int>*b,const int small_base)
{
	...
	else
	{
		if((int)b->size()>=2)
		{
			...
			for(int i=(int)a->size();i>=0;i--)if(f(a->front(),(*b)[0],small_base*(1+i*2))){fdis0=i;break;}
			for(int i=(int)a->size();i>=0;i--)if(f(a->front(),(*b)[1],small_base*(1+i*2))){fdis1=i;break;}
			for(int i=(int)a->size();i>=0;i--)if(f(a->back(),(*b)[0],small_base*(1+i*2))){bdis0=i;break;}
			for(int i=(int)a->size();i>=0;i--)if(f(a->back(),(*b)[1],small_base*(1+i*2))){bdis1=i;break;}
