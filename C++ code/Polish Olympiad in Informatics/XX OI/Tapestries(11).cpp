...
//	void Print()const{printf("vector(%lld,%lld)\n",x,y);}
};
struct Point
{
	int x,y;
...
struct Fraction
{
	...
	bool operator!=(const Fraction &f)const{return up*f.down!=f.up*down;}
	bool operator==(const Fraction &f)const{return up*f.down==f.up*down;}
//	void Print()const{printf("fraction(%lld,%lld)\n",up,down);}
};
struct Segment
{
	Point p1,p2;
...
bool GetHalfPlaneIntersection(vector<Segment>&segs,vector<Segment>&ans)
{
	...
	static Deq deq;
	deq.Clear();
	for(int i=1;i<(int)segs.size();i++)assert(segs[i-1].ToVector().IsOnRight(segs[i].ToVector()));
...
//	for(int i=0;i<(int)deq.Size();i++)printf("(%lld,%lld)(%lld,%lld)\n",deq[i].p1.x,deq[i].p1.y,deq[i].p2.x,deq[i].p2.y);
	for(bool changed=true;changed;)
	{
		changed=false;
		while(deq.Front(0).IntersectionDistance(deq.Front(1))<=deq.Front(0).IntersectionDistance(deq.Back(0)))deq.PopFront(),changed=true;
		while(deq.Back(0).IntersectionDistance(deq.Front(0))<=deq.Back(0).IntersectionDistance(deq.Back(1)))deq.PopBack(),changed=true;
	}
//	for(int i=0;i<(int)deq.Size();i++)printf("(%lld,%lld)(%lld,%lld)\n",deq[i].p1.x,deq[i].p1.y,deq[i].p2.x,deq[i].p2.y);
	assert(deq.Size()>=3);
...
//			printf("segs.size()=%lld\n",(int)segs.size());
			for(int i=0;i<(int)segs.size();i++)assert(segs[i].ToVector().IsOnRight(segs[(i+1)%segs.size()].ToVector()));
//			for(int i=0;i<(int)segs.size();i++)printf("(%lld,%lld)(%lld,%lld)\n",segs[i].p1.x,segs[i].p1.y,segs[i].p2.x,segs[i].p2.y);
		}
...
//		for(int i=0;i<(int)range.size();i++)printf("f=(%lld,%lld)\n",range[i].second.up,range[i].second.down);
		for(int i=0;i<(int)range.size();i++)
		{
			...
			for(int j=0;j<N;j++)
			{
				...
				if(!p.first.IsParallel(seg)&&seg.ExcludeIntersected(p.first))
				{
