...
int Dfs(const int u,const int parent,const Fraction &f)
{
	int ans=(f.up+f.down-1)/f.down;
	for(int i=0;i<(int)ET[u].size();i++)
	{
		const int nxt=ET[u][i];
		if(nxt!=parent)
		{
			const int ta=Dfs(nxt,u,f.Merge(Fraction((int)ET[nxt].size()-1,1)));
			if(ans<ta)ans=ta;
		}
	}
	return ans;
