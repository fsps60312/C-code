...
class Vector
{
	private:
		LL x,y;
		Vector(){}
		LL Cross(const Vector &p)const{return x*p.y-y*p.x;}//sin
		LL Dot(const Vector &p)const{return x*p.x+y*p.y;}//cos
	public:
...
struct Segment
{
	...
	Fraction IntersectionDistance(const Segment &l)const
	{
		...
		const LL a=p2.x-p1.x,b=-(l.p2.x-l.p1.x),c=l.p1.x-p1.x;
		const LL A=p2.y-p1.y,B=-(l.p2.y-l.p1.y),C=l.p1.y-p1.y;
		//at+bT=c,At+BT=C
	...
	bool InRange(const LL o,const LL a,const LL b)const{return (a<=o&&o<=b)||(b<=o&&o<=a);}
...
struct Deq
{
	...
	void PopBack(){R--;}
	void PopFront(){L++;}
	Segment Front(const LL i)const{return S[L+i];}
	Segment Back(const LL i)const{return S[R-i];}
	LL Size()const{return R-L+1;}
	Segment operator[](const LL i){return S[L+i];}
};
bool SHADOW[1000];
LL N;
...
//	freopen("inn.txt","r",stdin);
	LL testcount;scanf("%lld",&testcount);
	while(testcount--)
	{
		scanf("%lld",&N);
