...
int BCNT[1<<16];
int CntBits(const long long v){return BCNT[v>>48]+BCNT[(v>>32)&0x000000000000ffffLL]+BCNT[(v>>16)&0x000000000000ffffLL]+BCNT[v&0x000000000000ffffLL];}
const long long MOD1=(1<<22)-11603;
int N,K;
void ReadNK()
{
	char c=ReadChar();
...
long long START,END;
template<class LL>struct Solver2
{
...
//		printf("N=%d,K=%d\n",N,K);
		...
		puts((N<=22?Solve():(N<=30?(new Solver2<int>())->Run():(new Solver2<long long>())->Run()))?"TAK":"NIE");
	}
	return 0;
}
