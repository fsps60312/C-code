...
bool Solve()
{
	...
	if(bounds.size()==2)
	{
		...
		vector<Fraction>f;
		for(int i=0;i<n;i++)if(segs[i].ExcludeIntersected(shade))
		{
			f.push_back(shade.IntersectionDistance(segs[i]));
		}
		for(int i=0;i<n;i++)
		{
		...
		assert(f.size()==2);
		for(const auto v:f)if(v<Fraction(0,1)||Fraction(1,1)<v)return true;
		return false;
	}
	else
	{
		...
		const Fraction f=seg1.IntersectionDistance(seg2);
		for(int i=5;i<(int)bounds.size();i+=2)if(seg1.IntersectionDistance(Segment(P[bounds[i-1]],P[bounds[i]]))!=f)return false;
		bool *vis=new bool[N];
