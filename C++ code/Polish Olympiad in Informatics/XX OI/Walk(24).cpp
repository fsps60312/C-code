...
struct Solver2
{
	...
	bool Run()
	{
		...
		if(DISAPPEAR.Get(START)){assert(0);return false;}
		LL now=START;
		for(;;)
		{
//			printf("now=%lld\n",now);
			if(now==0LL)return true;
			const LL greedy=now^(now&(-now));
			assert(CntBits(greedy)==CntBits(now)-1);
			if(!DISAPPEAR.Get(greedy)){now=greedy;continue;}
			if(VIS.Set(now,true))return false;
//			puts("pass");
			queue<LL>q;
			assert(q.empty());
			assert(!DISAPPEAR.Get(now)),q.push(now);//,printf("push(%lld)\n",now);
			while(!q.empty())
			{
				const LL u=q.front();q.pop();
//				printf("u=%lld\n",u);
				assert(!DISAPPEAR.Get(u));
					...
					if(et)
					{
						...
						for(int j=0;j<sz;j++)
						{
							...
							if(k!=u)
							{
								...
								{
//									assert(CntBits(u^(u^(nxt^k)))==1&&CntBits((u^(nxt^k))^k)==1);
									if(!VIS.Set(k,true))assert(!DISAPPEAR.Get(k)),q.push(k);//,printf("push(%lld)\n",k);
									k=et->back();
									et->pop_back();
									j--,sz--;
								}
