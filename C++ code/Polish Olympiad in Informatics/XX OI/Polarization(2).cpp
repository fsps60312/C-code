...
int Solve(const map<int,int>&s)
{
	...
	{
		AddItems(vis,bound,it->first,it->second);
	}
...
LL Solve(const int u)
{
	...
	for(int i=0;i<(int)ET[u].size();i++)
	{
		...
		const Edge &e=EDGE[ei];
		const int nxt=(u==e.a?e.b:e.a);
		if(nxt==e.a)ans+=GetDP(ei*2),s[GetSZ(ei*2)]++;
		else ans+=GetDP(ei*2+1),s[GetSZ(ei*2+1)]++;
	}
	const LL half=Solve(s);
