...
bool Solve(const int tick)
{
	...
	static vector<int>enter[100002],leave[100002];
	for(int i=1;i<=M+1;i++)enter[i].clear(),leave[i].clear();
	for(int i=1;i<=N;i++)if(mn[i]!=INF)enter[mn[i]].push_back(i),leave[mx[i]+1].push_back(i);
	int certain=0,canleave=0,uncertain=0,leaved=0;
	for(int i=0,j=1;i<(int)counts.size();i++)
	{
		for(;j<=counts[i].first;j++)
		{
			for(int k=0;k<(int)enter[j].size();k++)
			{
				if(uncertain)uncertain--;
				certain++;
			}
			for(int k=0;k<(int)leave[j].size();k++)assert(certain),certain--,canleave++;
		}
