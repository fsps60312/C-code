...
bool Solve(const int tick)
{
	...
	if(true)
	{
		static vector<int>sot[100001];
		for(int i=1;i<=M;i++)sot[i].clear();
		for(int i=0;i<tick;i++)sot[T[i]].push_back(I[i]+1);
		for(int i=1;i<=M;i++)for(int j=0;j<(int)sot[i].size();j++)counts.push_back(make_pair(i,sot[i][j]));
	}
