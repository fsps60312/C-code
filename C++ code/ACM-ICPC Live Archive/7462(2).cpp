...
struct DisjointSets
{
	int DATA[10000],COUNT[10000],SAME[10000];
	void Clear(const int n){for(int i=0;i<n;i++)DATA[i]=i,COUNT[i]=1,SAME[i]=0;}
	...
	void Merge(int a,int b,bool get_max)
	{
		...
		DATA[a]=b;
		int count,same;
		if(get_max)count=max(COUNT[a],COUNT[b]),same=SAME[a]+SAME[b]+min(COUNT[a],COUNT[b]);
		else count=COUNT[a]+COUNT[b]+min(SAME[a],SAME[b]),same=max(SAME[a],SAME[b]);
		COUNT[b]=count,SAME[b]=same;
	}
}DJ;
char S[1000001];
