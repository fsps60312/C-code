#include<cstdio>
#include<cstdlib>
#include<vector>
#include<queue>
using namespace std;
struct Path{int to,dis;};
vector<int> INTE[1001];
vector<Path> PATH;
int WAY[1001],MAX[1001];
struct Trip
{
	int pos,dis,pre;
	bool operator<(Trip a)const{return dis>a.dis;}
};
int N;
int main()
{
	freopen("in.txt","r",stdin);
	while(scanf("%d",&N)==1&&N)
	{
		PATH.clear();
		for(int i=1;i<=N;i++)
		{
			INTE[i].clear();
			MAX[i]=2147483647;
		}
		int m;scanf("%d",&m);
		while(m--)
		{
			int a,b,dis;scanf("%d%d%d",&a,&b,&dis);
			INTE[a].push_back(PATH.size());
			PATH.push_back((Path){b,dis});
			INTE[b].push_back(PATH.size());
			PATH.push_back((Path){a,dis});
		}
		priority_queue<Trip> pq;
		WAY[0]=1;pq.push((Trip){1,0,0});
		WAY[2]=0;
		while(!pq.empty())
		{
			Trip trip=pq.top();pq.pop();
			int pos=trip.pos,dis=trip.dis,pre=trip.pre;
			if(dis>MAX[pos])continue;
			if(dis==MAX[pos]){WAY[pos]+=WAY[pre];continue;}
			if(dis<MAX[pos]){MAX[pos]=dis,WAY[pos]=WAY[pre];}
			for(int i=0;i<INTE[pos].size();i++)
			{
				Path p=PATH[INTE[pos][i]];
				pq.push((Trip){p.to,dis+p.dis,pos});
			}
		}
		printf("%d\n",WAY[2]);
	}
	return 0;
}
