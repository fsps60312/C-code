...
struct Matrix
{
	...
	void Sub(const int r1,const int r2){for(int i=0;i<=n;i++)s[r2][i]-=s[r1][i];}
	void Eliminate(const int r1,const int r2,const int c)
	{
		if(Zero(s[r2][c]))return;
		Mul(r2,s[r1][c]/s[r2][c]);
		Sub(r1,r2);
	}
};
...
bool Solve(const int n,vector<double>&ans)
{
	...
	ans.clear();
	for(int i=0;i<n;i++)assert(!Zero(s[i][i])&&s[i][i]==s[i][i]),ans.push_back(s[i][n]/s[i][i]);
	for(int i=n;i<L;i++)if(!Zero(S[i]-GetValue(ans,i)))return false;
	return true;
}
void Solve()
{
	vector<double>ans;
	bool found=false;
	for(int n=2;n<=L;n++)if(Solve(n,ans)){found=true;break;}
	assert(found&&"Solve");
