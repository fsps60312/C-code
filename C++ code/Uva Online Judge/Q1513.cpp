...
int max(int a,int b,int c){return a<b?max(b,c):max(a,c);}
int min(int a,int b,int c){return a<b?min(a,c):min(b,c);}
struct Node
{
	Node *ch[2];
	...
	Node(){}
	Node(int idx):idx(idx),sz(1),height(1),minq(0),q(queue<int>()){ch[0]=ch[1]=NULL;}
}*ROOT,NODE[100001];
int Sz(Node* &v){return v?v->sz:0;}
int Height(Node* &v){return v?v->height:0;}
int Q(Node* &v){return v?v->GetQ():2147483647;}
int Idx(Node* &v){return v?v->idx:-1;}
void show(Node* &o)
{
	if(!o)return;
	show(o->ch[0]);
	printf("{%d:(%d,%d)(%d,%d,%d)",Idx(o),Idx(o->ch[0]),Idx(o->ch[1]),Sz(o),Height(o),Q(o));
	queue<int>q=o->q;
	while(!q.empty())
	{
		printf(",%d",q.front());
...
void Maintain(Node* &n)
{
	...
	n->minq=min(Q(n),Q(n->ch[0]),Q(n->ch[1]));
}
...
{
	int a=Q(o),b=Q(o->ch[0]),c=Q(o->ch[1]);
//	printf("%d:%d %d:%d %d:%d\n",Idx(o),a,Idx(o->ch[0]),b,Idx(o->ch[1]),c);
	int v=min(a,b,c);
	if(v==2147483647)termin("v==%d\n",v);
	if(v==b||v==c)
	{
