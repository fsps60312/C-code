...
struct treaptype
{
	...
	void insert(vertextype* &a,vertextype *b)
	{
		...
		if(a==NULL){a=b;b->maintain();return;}
		int d=a->cmp(b->v);
		insert(a->ch[d],b);
		a->maintain();
	...
	void insert(vertextype *b){insert(root,b);}
	...
	int qvbyrank(vertextype *a,int rank)
	{
		...
		if(rank<b)return qvbyrank(a->ch[1],rank);
		return qvbyrank(a->ch[0],rank-b);
	}
