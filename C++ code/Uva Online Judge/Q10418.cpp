...
int GetDis(const int source,const int direction,const int target)
{
	VIS_CNT++;
	...
	while(!pq.empty())
	{
		...
		if(VIS[p.u][p.d]==VIS_CNT)continue;
		VIS[p.u][p.d]=VIS_CNT;
		if(p.u==target)return p.cost;
...
//	freopen("in.txt","r",stdin);
	for(int i=0;i<10000;i++)VIS[i][0]=VIS[i][1]=0;VIS_CNT=0;
	...
	while(casecount--)
	{
		...
		for(int i=0;i<N*2;i++)
		{
			for(int j=0;j<=N*2;j++)HUNGARY.EDGE[i][j]=GetDis(SOLDIER[i],i<N?1:0,TARGET[j]);
		}
		HUNGARY.N=N*2;
		int lans=0,rans=N*2;
		while(lans<rans)
