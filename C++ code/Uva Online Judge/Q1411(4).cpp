...
inline void getmin(double &a,const double b){if(b<a)a=b;}
int N;
struct Hungary
{
	double DIST[100][100];
	...
	bool CheckPaired(const int source)
	{
		assert(!AVIS[source]);
		AVIS[source]=true;
		for(int i=0;i<N;i++)if(!BVIS[i])
		{ 
			if(DIST[source][i]-WA[source]-WB[i]<EPS)
			{
			...
			else getmin(LOW[i],DIST[source][i]-WA[source]-WB[i]);
		} 
		return false;
	}
	void Update()
	{
		double low=INF;
		for(int i=0;i<N;i++)if(!BVIS[i])getmin(low,LOW[i]);
		for(int i=0;i<N;i++)
		{
			if(AVIS[i])WA[i]+=low;
			if(BVIS[i])WB[i]-=low;
	...
	void Solve()
	{
		...
		for(int source=0;source<N;source++)if(AT[source]==-1)
		{
			for(;;)
			{
				for(int i=0;i<N;i++)AVIS[i]=BVIS[i]=false,LOW[i]=INF;
