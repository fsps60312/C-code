...
{
	Cable CBL[10000];
	int CSZ,USZ,PRE[60],LOW[60],PRE_CNT,BCC_CNT,COST[60],VIS[60],BCCN[60];
	vector<int> BCC[60];
	void init(Cable *c,int length,int limit,int usz)
	{
		CSZ=0;
		USZ=usz;
		for(int i=0;i<length;i++)
		{
			if(c[i].bw<limit||c[i].to==0)continue;
			CBL[CSZ++]=c[i];
		}
	}
...
bool BuildCable(int limit)
{
	...
	for(int i=0;;i^=1)
	{
		...
		DMST[i^1].init(DMST[i].BCC_CABLE,DMST[i].BCC_CSZ,limit,DMST[i].BCC_CNT);
