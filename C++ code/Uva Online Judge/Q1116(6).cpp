		...
		while(T--)
		{
			...
			if(!iscyc)
			{
				...
				while(!q.empty())
				{
					...
					assert(ALL.find(Edge(a,b))!=ALL.end());
					DeleteEdge(a,b);
//for(map<Edge,int>::iterator it=ALL.begin();it!=ALL.end();it++)printf(" %d %d\n",it->first.a,it->first.b);
					M--;
			...
			for(int i=0;i<ANS.size();i++)assert(ANS[i]==i+1);
			if(T)puts("");
		}
	}
	return 0;
