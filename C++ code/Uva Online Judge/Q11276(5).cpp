...
struct Matrix
{
	int s[114][114];
	int n;
	Matrix(const int _n):n(_n){/*printf("n=%d\n",n);*/assert(n<=114);}//{s.clear();s.resize(n,vector<int>(n));}
...
//	for(int i=1;i<=14;i++)
//	{
//		Matrix a=Pow(A_SOLVER.M,i),b=Pow(B_SOLVER.M,i),c=Pow(C_SOLVER.M,i);
//		printf("pow %d: %d %d %d\n",i,a.Used(),b.Used(),c.Used());
//	}
