...
struct Trie
{
	struct Node
	{
		...
		Node():v(""),vis(0){for(int i=0;i<10;i++)bse[i]=0;}
	};
	vector<Node>bse;
	int sz;
	...
	bool changevalue(int u,string &v)
	{
		if(bse[u].vis)return false;
		bse[u].vis=1;
		bse[u].v=v;return true;
	}
	void insert(char *s1,char *s2,int idx,int u,int _state,string &v)
	{
		idx++;
		if(bse[u].v.size())return;
		if(!s1[idx])
		{
