...
void PushToA(int idx,int s)
{
	if(idx==N)
	{
		...
		if(A[a].size()>=N*N)
		{
			if(ERROR!=-1&&a!=ERROR)return;
			ERROR=a;
			printf("error N=%d\n",N);
			for(int i=0;i<A[a].size();i++)if(A[a][i]==s)printf("findsame");
			for(int k=0;k<N;k++)
			{
				printf("%d",s&1);
				s>>=1;
...
int main()
{
	...
	while(scanf("%d",&N)==1&&N)
	{
		...
		for(int i=0;i<N*N;i++)for(int j=0;j<=W;j++)for(int k=0;k<=H;k++)DP[j][k][i]=-1;
		sort(P,P+N);
		PushToA(0,0);
		if(ERROR!=-1)
		{
		...
		printf("Case %d: ",kase++);
		printf(sum==W*H&&getDP(W,H,0)?"Yes\n":"No\n");
	}
	return 0;
}
