...
vector<double> solve(vector<double> &eqt,int dep)
{
	...
	{
		vector<double> ans;
		if(abs(eqt[0]/eqt[1]<=1.0))ans.push_back(-eqt[0]/eqt[1]);
		return ans;
	}
	...
	else ans.push_back(aprox(eqt,-1.0,1.0));
	vector<double> ta;
	ta.push_back(ans[0]);
	for(int i=1,j=0;i<ans.size();i++)
	{
		if(abs(ans[i]-ans[i-1])*E<ER)ta[j]=aprox(eqt,ans[i-1],ans[i]);
		else ta.push_back(ans[i]),j++;
	}
	ans.clear();
	for(int i=0;i<ta.size();i++)if(abs(getvalue(eqt,ta[i]))/E/E/E/E<ER)ans.push_back(ta[i]);
//	printf("%d: %d\n",dep,ans.size());
	return ans;
}
int main()
...
//		printf("sin: sz=%d",sin.size());for(int i=0;i<sin.size();i++)printf(" %.5f",sin[i]);puts("");
		for(int i=0;i<sin.size();i++)
		{
			for(int j=0;j<cos.size();j++)
			{
				...
				if(abs((x-P)*dx+(y-Q)*dy)/E/E<ER)printf("%.5f %.5f\n",x,y);
			}
		}
	}
	return 0;
