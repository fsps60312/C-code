...
//	printf("lq=%.3f,angle=%.3f\n",lq,angle/PI*180);
	if(h+EPS<dis)
	{
		const double bngle=-acos(dis/lq)+atan(r/h);
...
//	printf("dis=%.2f,r2=%.2f,r1=%.2f,h1=%.2f,offset=%.3f\n",dis,r2,r1,h1,offset/PI*180);
	double angle;
	if(h1>=sqrt(dis*dis-Sq(r1+r2)))angle=asin((r1+r2)/dis);
	else angle=acos((dis*dis+lq*lq-r2*r2)/(2.0*dis*lq))+atan(r1/h1);
//	printf("lq=%.3f,angle=%.3f\n",lq,angle/PI*180);
	if(h1+EPS<dis-r2)
	{
		const double bngle=acos((r2+h1)/dis);
//		if(!(0.0<=bngle&&bngle<=angle))
//		{
//			printf("dis=%.2f,r2=%.2f,r1=%.2f,h1=%.2f,offset=%.3f\n",dis,r2,r1,h1,offset/PI*180);
//			printf("lq=%.3f,angle=%.3f,bngle=%.3f\n",lq,angle/PI*180,bngle/PI*180);
//		}
		assert(0.0<=bngle&&bngle<=angle);
		ans.push_back(AngleRange(-angle,-bngle).Offset(offset));
		ans.push_back(AngleRange(bngle,angle).Offset(offset));
...
bool CanCut(const int u)
{
	...
	GetFenceShadow(tmp,YMX-S[u].Y,0.5*S[u].D,S[u].H,0.5*PI);
	GetFenceShadow(tmp,S[u].X-XMN,0.5*S[u].D,S[u].H,1.0*PI);
	GetFenceShadow(tmp,S[u].Y-YMN,0.5*S[u].D,S[u].H,1.5*PI);
...
//		for(int i=0;i<N;i++)printf("CanCut(%d)=%d\n",i,CanCut(i));return 0;
		...
		{
			changed=false;
			for(int j=0;j<N;j++)if(CanCut(j))++ans,changed=true,EXIST[j]=false;//,printf("tree=%d\n",j);
//			break;
		}
		static int kase=0;
		printf("Forest %d\n%d tree(s) can be cut\n\n",++kase,ans);
