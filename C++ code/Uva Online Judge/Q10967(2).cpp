...
{
//	printf("Edge(%lld,%s)\n",cost,directed?"directed":"undirected");
...
bool Initialize()
{
	...
	assert(GetIdx(row-1,0,0)==0&&GetIdx(0,column-1,0)==1);
	for(int i=0;i<(int)dr.size();i++)
	{
		...
		if(outr>=0&&outr<row&&outc>=0&&outc<column)
		{
			if(grid[outr][outc]=='.')AddEdge(GetIdx(outr,outc,0),GetIdx(r,c,d),1LL,true);
			for(int disp=1;disp<=3;disp++)
			{
				...
				{
					AddEdge(GetIdx(r,c,(d+disp)%4),GetIdx(tr,tc,grid[tr][tc]=='.'?0:(d+disp+2)%4),1LL,true);
				}
			}
		}
	}
...
int main()
{
	...
	while(casecount--)
	{
		const LL ans=(Initialize()?Solve():0LL);
...
EW.#.N.
....WS.
4 7 6 10 9 1 7 5 8 6 5 8 10 6 10 10 7 10 6 5 10
*/
