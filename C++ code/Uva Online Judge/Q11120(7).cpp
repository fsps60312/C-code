...
int Pop(){const int ans=BUFFER.front();BUFFER.pop();printf("Pop %d\n",ans);return ans;}
...
//	freopen("in.txt","r",stdin);
	...
	while(scanf("%d",&N)==1)
	{
		...
		for(;;)
		{
			printf("DEQ(%d,%d): ",DEQ.l,DEQ.r);DEQ.Print();
			int l=0,r=9;
			while(l<r)
			{
			...
			ADD[0]=0;
			printf("ADD: ");for(int i=(int)ADD.size()-1;i>=0;i--)putchar('0'+ADD[i]);puts("");
			ans.push_back(r);
			if(FindAns(ans))
			{
				printf("ans: ");for(int i=0;i<ans.size();i++)putchar('0'+ans[i]);puts("");
