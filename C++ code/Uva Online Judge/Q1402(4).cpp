...
{
	LL h,p;
	bool operator<(Testtube a)
	{
		if(h!=a.h)return h<a.h;
		return p<a.p;
	}
	bool operator==(Testtube a){return h==a.h&&p==a.p;}
};
Testtube newtube(LL h,LL p){Testtube ans;ans.h=h,ans.p=p;return ans;}
struct Node
{
	LL f,sum;
	Testtube h,m;
...
void push_back(Node* &o,Testtube h)
{
	//printf("a\n");
	if(o==NULL)
	{
...
void splay(Node* &o,LL r)
{
	putdown(o);
	LL tr=1;if(o->ch[0]!=NULL)tr+=o->ch[0]->sum;
	if(r==tr)return;
	LL d=0;
	if(r>tr)d=1,r-=tr;
	splay(o->ch[d],r);
...
LL findmin(LL p)
{
	...
	//sink(root);
	LL ans=p-1+querypos(root);
	//splay(root,1);
	root->ch[0]=l;root->maintain();
	sink(root);
	return ans;
}
void rev(LL lp,LL rp)
{
	splay(root,rp);
