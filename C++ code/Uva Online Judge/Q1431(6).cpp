...
//			printf("left:  ");left.Print();puts("");
//			printf("right: ");right.Print();puts("");
//			printf("d=%d,digit=%d\n",d,digit);
			assert(digit>=0&&digit<=9);
			result.push(digit);
			right-=(left+digit)*digit;
			left+=digit,left+=digit,left*=10;
		}
		if(right.s.size()>1||right.s[0]!=0)return false;
//		printf("right: ");right.Print();puts("");
		ans.s.clear();
...
Big N,D;
int E;
bool GetSub(const Big &a,Big n,Big &sub)
{
	Big tmp=a*a;
	n*=4;
...
bool Solve(const Big &pq,Big &p,Big &q)
{
...
//	printf("add: ");add.Print();puts("");
	static Big sub;
	if(!GetSub(add,N,sub))return false;
	Big tmp=add;tmp-=sub;
	if(!tmp.DivideBy(2,p))return false;
	tmp=add;tmp+=sub;
	if(!tmp.DivideBy(2,q))return false;
	return true;
}
int main()
{
...
//		printf("D*E-1: ");ALL.Print();puts("");
		static Big p,q;
		for(int n=E-1;;n--)if(ALL.DivideBy(n,pq)&&Solve(pq,p,q))break;//summary (p-1)(q-1)
		static int kase=1;
		printf("Case #%d: ",kase++);
