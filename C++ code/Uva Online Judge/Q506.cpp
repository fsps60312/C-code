...
int IN[MAXN],INSTED[MAXN],INSTIME_CNT,INSTIME[MAXN];
...
void install(int u)
{
	...
	for(int i=0;i<LINK[u].size();i++)
	{
		...
		//printf("%d->%d\n",u,j);
		IN[j]++;
		if(!INSTED[j])install(j);
	}
	printf("   Installing %s\n",NAME[u].c_str());
	INSTED[u]=1;
	INSTIME[u]=INSTIME_CNT++;
}
void install(char *name){I=0;int u=getidx(name);install(u);}
void remove(int u)
{
