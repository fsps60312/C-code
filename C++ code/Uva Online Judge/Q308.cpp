...
void Bfs(const int sy,const int sx)
{
	...
	while(!qy.empty())
	{
		...
		if(VIS[y][x])continue;
		VIS[y][x]=true;
		if(!CUT[y][x][0])qy.push(y-1),qx.push(x);
		if(!CUT[y][x][1])qy.push(y),qx.push(x+1);
		if(!CUT[y][x][2])qy.push(y+1),qx.push(x);
		if(!CUT[y][x][3])qy.push(y),qx.push(x-1);
	}
}
int main()
{
//	freopen("in.txt","r",stdin);
	for(;;)
	{
		...
		ReIndex();
		int ans=-1;
		for(int i=0;i<=(int)YIDX.size();i++)for(int j=0;j<=(int)XIDX.size();j++)if(!VIS[i][j])Bfs(i,j),ans++;
		assert(ans>-1);
		printf("%d\n",ans);
	}
	assert(scanf("%d",&N)!=1);
