...
nodetype *root[20001];
int side[60001][3],qv[100000][2],Q,leader[20001];
char qtype[100000];
int findset(int a)
{
...
int main()
{
    ...
    while(scanf("%d%d",&N,&M)==2&&(N||M))
    {
        ...
        for(int i=1;i<=M;i++)
        {
            ...
            if(tside.find(newpair(side[i][0],side[i][1]))==tside.end())tside[newpair(side[i][0],side[i][1])]=1;
            else tside[newpair(side[i][0],side[i][1])]++;
            side[i][2]=0;
        }
        Q=0;
        while(scanf("%c",&qtype[Q])==1&&qtype[Q]!='E')
        {
            switch(qtype[Q])
            {
                case'C':
                {
                    scanf("%d",&qv[Q][0]);//printf("%d ",qv[Q][0]);
                    qv[Q][1]=vertex[qv[Q][0]];
                    scanf("%d",&vertex[qv[Q][0]]);//printf("%d\n",vertex[qv[Q][0]]);
                    root[qv[Q][0]]->v=vertex[qv[Q][0]];
                }break;
                case'Q':
                {
                ...
                {
                    scanf("%d",&qv[Q][0]);
                    if(--tside[newpair(side[qv[Q][0]][0],side[qv[Q][0]][1])])break;
                    side[qv[Q][0]][2]=1;
                }break;
                default:
                {
