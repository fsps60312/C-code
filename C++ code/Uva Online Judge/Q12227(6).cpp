...
int Solve()
{
	...
	for(int i=0;i<N;i++)
	{
		for(;;)
		{
			...
			{
				Warmhole &w1=WARMHOLE[j];
				Warmhole &w2=WARMHOLE[k];
				if(k==-1||(DT[j]-(w1.creation+w1.shift)<DT[k]-(w2.creation+w2.shift)))k=j;
			}
			assert(k!=-1);
