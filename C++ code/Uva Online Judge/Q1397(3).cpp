...
void solve()
{
	for(int i=0;i<S;i++)
	{
		int j=i;for(int k=j+1;k<S;k++)if(abs(MAT[k][i])>abs(MAT[j][i]))j=k;
		if(j==S)termin("j==N");
		Exchange(i,j);
		for(int k=0;k<S;k++)
		{
			...
			for(int t=0;t<S;t++)if(Zero(MAT[k][t]))MAT[k][t]=0.0;
		}
	}
	for(int i=0;i<S;i++)
	{
...
//	freopen("out.txt","w",stdout);
	...
	while(scanf("%lf%d%lf%d",&A,&M,&B,&N)==4&&(A||M||B||N))
	{
		...
		solve();
		printf("1");
		for(int i=S-1;i>=0;i--)printf(" %lld",(LL)(MAT[i][S]+(MAT[i][S]>0.0?0.5:-0.5)));
		puts("");
	}
	return 0;
}
