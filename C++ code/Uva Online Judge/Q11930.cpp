...
LL Cross(const Pair &a,const Pair &b){return (LL)a.x*b.y-(LL)b.x*a.y;}
...
//	freopen("in.txt","r",stdin);
	for(;;)
	{
		...
		{
			static int x[4],y[4];
			for(int j=0;j<4;j++)assert(scanf("%d%d",&x[j],&y[j])==2);
			sort(x,x+4),sort(y,y+4);
			assert(x[0]==x[1]&&x[2]==x[3]&&y[0]==y[1]&&y[2]==y[3]);
			SEGMENT[i][0]=Segment(Pair(x[0],y[0]),Pair(x[2],y[2]));
			SEGMENT[i][1]=Segment(Pair(x[0],y[2]),Pair(x[2],y[0]));
		}
		SOLVER.clear(N*2);
