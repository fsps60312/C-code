...
inline void UpdateValid(Building &b1,Building &b2,int len)
{
	...
	for(x1=0;x1<2;x1++)for(y1=0;y1<2;y1++)
	{
		...
		if(cnt==0)AddSat(b1.Idx(x1,y1),true,b2.Idx(0,0),true);//(x1,y1) is invalid
		else if(cnt==1)AddSat(b1.Idx(x1,y1),true,b2.Idx(x,y),true);
	}
}
bool solve()
{
	...
	{
		Edge &e=EDGE[i];
		UpdateValid(B[e.a],B[e.b],e.v);
		UpdateValid(B[e.b],B[e.a],e.v);
	}
