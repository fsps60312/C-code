...
//			case 7:return Point(x%hN+(y&1)*hN,(y>>1<<1)+(x>=hN));
			case 7:return Point((x>>1)+(y&1)*hN,(y>>1<<1)+(x&1));
...
LL GetCycle(Point p)
{
	...
	{
//		if(VIS[m.x][m.y]){printf("%d %d %d %d\n",p.x,p.y,m.x,m.y);return -1LL;}
		VIS[m.x][m.y]=1;
		if(m==p)return ans;
	}
}
...
//			printf(" %lld",b);if(b<0LL)return -1LL;
			ans=lcm(ans,b);
		}
	}
//	puts("");
...
//		for(int i=0;i<TRAN.size();i++)printf(" %d",TRAN[i]);puts("");
		printf("%lld\n",solve());
		if(T)puts("");
	}
}
