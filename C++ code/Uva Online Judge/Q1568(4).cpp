...
void Solve()
{
	...
	for(int i=0;i<(int)G.size();i++)
	{
		...
		else
		{
			const int sz=s.size();
			assert(sz==2||sz==4);
			END.push_back(Pair(s[sz-2],s[sz-1]));
		}
	}
	...
	for(int i=0;i<(int)G.size();i++)
	{
		...
		{
			vector<Pair>node;
			node.push_back(Pair(s[0],s[0]));
			for(int j=0;j<(int)G.size();j++)if(j!=i)node.push_back(END[j]);
			node.push_back(Pair(s[1],s[1]));
			for(int i=1;i<(int)node.size();i++)TA.push_back(Pair(node[i-1].a,node[i].b));
			minus=s[2]+s[3];
//			printf("(%d,%d)\n",s[0],s[1]);
//			for(int i=0;i<TA.size();i++)printf("(%d,%d)\n",TA[i].a,TA[i].b);
			end1=end2=-2;
			break;
		}
	}
