...
//		printf("u=%d,i=%d,cost=%d,pre=%d,t=%d\n",RI[p.u],p.i,p.cost,p.pre,p.t);
		...
		for(int i=0;i<AD[p.u].size();i++)
		{
			...
			for(int i=1;i<t.city.size()&&pt.i<trip.size();i++)
			{
				...
				pt.u=c;
				bool passby=false;
				if(pt.u==trip[pt.i])pt.i++,passby=true;
				pq.push(pt);
...
//	printf("n=%d u=%d\n",n,u);
	...
	if(TICK[n][u]!=-2)ANS.push_back(TICK[n][u]+1);
}
int main()
{
...
ticket.push_back(t);
		...
		while(NI--)
		{
			...
			vector<int>ta;
			ta.push_back(ANS[0]);
			for(int i=1;i<ANS.size();i++)if(ANS[i]!=ANS[i-1])ta.push_back(ANS[i]);
			ANS=ta;
