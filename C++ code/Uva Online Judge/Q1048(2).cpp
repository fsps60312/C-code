...
int LASTPOS[21][401],TICKETN[21][401],TICKETI[21][401];
Trip BEST[21][401];
Trip solve(vector<int> &sche)
{
	...
	for(int i=1;i<=20;i++)
	{
		for(int t=1;t<=400;t++)
		{
			...
			BEST[i][t]=(Trip){-1,-1,2147483647,2147483647};
		}
	}
	...
	while(!pq.empty())
	{
		...
		if(t<BEST[t.pos][t.num])continue;
		BEST[t.pos][t.num]=t;
		...
		for(int i=0;i<TICKET[t.pos].size();i++)//each ticket from t.pos
		{
			...
			for(int u=0;u<tkt.to.size();u++)//each position this ticket will go
			{
				...
				pq.push((Trip){tkt.to[u],next,t.cost+tkt.cost,t.num+1,t.pos,tkt.idx,u});
