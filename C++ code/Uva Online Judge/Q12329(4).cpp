...
//				printf("%d %d\n",ld,rd);
				if(ld&&(now->v+1>=c||ld->v<=c+C))for(int i=max(ld->v+1,c);i<min(now->v,c+C);i++)ANS[i-c][y-r]='-';
//				puts("a");
				if(rd&&(now->v<=c+C||rd->v+1>=c))for(int i=max(now->v+1,c);i<min(rd->v,c+C);i++)ANS[i-c][y-r]='-';
//				puts("b");
			}
//			now=o;
//			for(int i=0;now&&i<C;i++,now=now->r)
