...
void Circle(int &xc,int &yc,int &r,int &c)
{
	...
	{
		y=yc+dy;
		for(int x=findDJ(y,xc+max(-xc,-(int)sqrt(r*r-dy*dy))),limit_x=xc+min(N-1-xc,(int)sqrt(r*r-dy*dy));x<=limit_x;x=findDJ(y,x))
		{
			ANS[c]++;
			W[y][x]=x+1;
		}
...
void Diamond(int &xc,int &yc,int &r,int &c)
{
	...
	{
		y=yc+dy;
		for(int x=findDJ(y,xc+max(-xc,-(r-abs(dy)))),limit_x=xc+min(N-1-xc,r-abs(dy));x<=limit_x;x=findDJ(y,x))
		{
			ANS[c]++;
			W[y][x]=x+1;
		}
...
{
	for(int dy=0,limit_dy=min(M-1-yc,w-1),y;dy<=limit_dy;dy++)
	{
		y=yc+dy;
		for(int x=findDJ(y,xc),limit_x=xc+min(N-1-xc,l-1);x<=limit_x;x=findDJ(y,x))
		{
			ANS[c]++;
			W[y][x]=x+1;
		}
...
void Triangle(int &xc,int &yc,int &w,int &c)
{
	...
	{
		y=yc+dy;
		for(int x=findDJ(y,xc+max(-xc,-((w-1)/2-dy))),limit_x=xc+min(N-1-xc,(w-1)/2-dy);x<=limit_x;x=findDJ(y,x))
		{
			ANS[c]++;
			W[y][x]=x+1;
		}
