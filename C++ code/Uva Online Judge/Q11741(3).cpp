...
//	}
	...
	{
		const bool usable=Usable(row,col),usecol=Usable(row,col+1),userow=Usable(row+1,col);
//		printf("row=%d,col=%d\n",row,col);
		for(int s=0;s<(1<<C);s++)if(DP[d][s])
		{
			const LL &value=DP[d][s];
			...
			else
			{
				...
				{
					static int target;
					if(usecol&&TryH(s,target,col))Add(DP[d^1][target],value);
					if(userow&&TryV(s,target,col))Add(DP[d^1][target],value);
				}
			}
			DP[d][s]=0LL;
		}
