...
{
	char carry=0;
//	printf("sub:");for(int i=0;i<d.size();i++)printf("%d",d[i]);printf(",");for(int i=0;i<m.size();i++)printf("%d",m[i]);puts("");
...
//	for(int i=0;i<rem.size();i++)printf(" %d",rem[i]);puts("");
	if(!n.size())termin("!n.size()\n");
//	printf("pre n=");show(n);puts("");
	castdigit(sub,n);
	if(n.size()%2)castdigit(sub,n);
	char v,now=-1,cnt=0;
	int pos=0;
//	printf("post n=");show(n);puts("");
//	printf("sub=");show(sub);puts("");
	while(1)
	{
		add.push_front(0);
		v=0;
		while(v<9&&Nolarger(tmp=Addmul(add,v+1),sub))v++;
...
int main()
{
//	Deq d;show(d);char idx=0;puts("");
//	d.push_front(idx);show(d);idx=(idx+1)%10;puts("");
//	d.push_front(idx);show(d);idx=(idx+1)%10;puts("");
//	d.push_front(idx);show(d);idx=(idx+1)%10;puts("");
//	d.push_front(idx);show(d);idx=(idx+1)%10;puts("");
//	d.push_back(idx);show(d);idx=(idx+1)%10;puts("");
//	d.push_back(idx);show(d);idx=(idx+1)%10;puts("");
//	d.push_front(idx);show(d);idx=(idx+1)%10;puts("");
//	d.push_front(idx);show(d);idx=(idx+1)%10;puts("");
//	d.push_back(idx);show(d);idx=(idx+1)%10;puts("");
