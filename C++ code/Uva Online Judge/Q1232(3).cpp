...
{
	if(t1.MIN!=-2147483647&&t2.MIN!=-2147483647)
	{
	...
	{
		if(t1.MIN!=-2147483647)a=t1;
		else a=t2;//because t2.MIN!=-2147483647
	}
}
int change(int id,int l,int r,int L,int R,int H)
{
	...
	if(l>=L&&r<=R)
	{
		if(ST[id].MAX<=H||ST[id].MIN==-217483647)
		{
