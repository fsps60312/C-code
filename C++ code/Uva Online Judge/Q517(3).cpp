...
inline int Transform(const int &a,const int &r)
{
	static int ans;ans=a;
	for(int i=0;i<N;i++)if(Bit(a,i)==C[r][0]&&Bit(a,(i+2)%N)==C[r][1]&&Bit(a,(i+3)%N)==C[r][2])SetBit(ans,(i+2)%N,C[r][3]);
	return ans;
}
inline int Transform(const int &a)
{
	static int r,ans;ans=a;
	for(r=0;r<8;r++)ans=Transform(ans,r);
	return ans;
}
int Solve()
{
