...
//1/p==1+E^(-a-bn)
	while(scanf("%d",&N)==1&&N)
	{
		...
		double x=0.0,y=0.0,dis=100.0,ratio=0.999,k=0.1,angle,dx,dy,dv;
		double value=getvalue(x,y),v;
		while(dis>EPS)
		{
			...
			dv=v-value;
			if(v<value/*||randdouble()<=exp(-dv/(k*dis))*/)x+=dx,y+=dy,value=v;
			dis*=ratio;
		}
		printf("%.4f %.4f\n",x,y);
	}
