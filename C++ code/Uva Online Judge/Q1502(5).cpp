...
bool GetLine(char *tmp)
{
	for(;;)
	{
		if(!fgets(tmp,INF,stdin))return false;
		for(int i=0;tmp[i]&&tmp[i]!='\n';i++)if(Digit(tmp[i]))return true;
	}
}
void assert(bool valid){if(valid)return;for(;;)putchar('E');}
void bssert(bool valid){if(valid)return;exit(0);}
...
//	freopen("out.txt","w",stdout);
	...
	assert(GetLine(tmp)&&sscanf(tmp,"%d",&T)==1);
	while(T--)
	{
		assert(GetLine(tmp)&&sscanf(tmp,"%d",&N)==1);
