...
int Solve()
{
	...
	if(zero_taken==0&&li%2==1)
	{
		if(li==(int)LEFT.size()&&ri==(int)RIGT.size())
		{
			...
			&&(li==(int)LEFT.size()||/*RIGT[ri]/LEFT[li-1]>=LEFT[li]/RIGT[ri-1]*/RIGT[ri]*RIGT[ri-1]>=LEFT[li]*LEFT[li-1]))++ri,--li;
		else if(ri>0&&li<(int)LEFT.size()
			&&(ri==(int)RIGT.size()||/*LEFT[li]/RIGT[ri-1]>=RIGT[ri]/LEFT[li-1]*/LEFT[li]*LEFT[li-1]>=RIGT[ri]*RIGT[ri-1]))--ri,++li;
		else assert(0);
	}
	if(zero_taken!=0)
	{
