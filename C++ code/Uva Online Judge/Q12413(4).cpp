...
BigInt floor(Big &a)
{
	BigInt ans=BigInt(a.v);
	if(a.dcmplc<0)ans=ans.DigitsMoveLeft(-a.dcmplc);
	else if(a.dcmplc>0)
	{
		for(int i=0;i<a.dcmplc;i++)ans.d[i]=0;
		ans=ans.DigitsMoveRight(a.dcmplc);
	}
	return ans;
}
Big pow(Big a,BigInt b)
{
	Big ans=Big(1LL);
	BigInt two=BigInt(2LL);
	while(!!b)
	{
		if(b.d[0]&1)ans*=a;
		a*=a;
...
Big pow(Big a,Big b)
{
	Big two=Big(2LL),eps=Big(1LL);
	BigInt flr=floor(b);
	Big ans=pow(a,flr);
	Big tmp;tmp.v=flr,tmp.dcmplc=0;
	b-=tmp;
	eps.dcmplc=a.accuracy+RESERVED;
	tmp=Big(1LL)/two;
	Big now=sqrt(a);
	while(tmp>eps)
	{
...
//	freopen("out.txt","w",stdout);
	...
	{
		if(IDX.find(LINE)==IDX.end())termin("Illegal Operator\n");
		...
		switch(type)
		{
			...
			case 4:show(pow(BIG1,BIG2));break;
...
BigInt sqrt(BigInt a)
{
	if(!a)return BigInt(0LL);
	BigInt n[10];for(LL i=0;i<10LL;i++)n[i]=BigInt(i);
	BigInt now=BigInt(1LL).DigitsMoveLeft((a.d.size()+1)/2*2),tmp(0LL);
	BigInt ans=BigInt(1LL).DigitsMoveLeft((a.d.size()-1)/2);
	for(int i=(a.d.size()-1)/2;i>=0;i--)
	{
		now=now.DigitsMoveRight(2);
		if(!!tmp)tmp=tmp.DigitsMoveRight();
		char &v=ans.d[i]=0;
		while(v<9&&(now*n[v+1]+tmp)*n[v+1]<=a)v++;
		BigInt iii=(now*n[v]+tmp)*n[v];
		tmp=now*n[v]*n[2]+tmp;
		a-=iii;
	}
	return ans;
...
Big sqrt(Big a)
{
	int dis=(a.accuracy+RESERVED)*2;
	if(a.dcmplc%2)dis++;
	a.v=a.v.DigitsMoveLeft(dis);
	a.v=sqrt(a.v);
//	printf("a.v=");a.v.printf();puts("");
	a.dcmplc+=dis;
	a.dcmplc/=2;
	return a;
}
