...
int Solve()
{
	...
	{
		Pq p=pq.top();pq.pop();
		if(PVS[p.r][p.c][p.len]==kase)continue;
//		printf("%d %d %d %d\n",p.r,p.c,p.len,p.cost);
		PVS[p.r][p.c][p.len]=kase;
...
//			if(COST[p.r][p.c][i]==INF)continue;
			int cost;
			if(y==E.r&&x==E.c)return p.cost;
			else if(COST[p.r][p.c][i]==INF)continue;
			else cost=max(p.cost,p.len+COST[p.r][p.c][i]);
...
//	freopen("in.txt","r",stdin);
	for(int i=0;i<40;i++)for(int j=0;j<40;j++)for(int k=0;k<40*40;k++)PVS[i][j][k]=0;
	while(scanf("%d",&T)==1)
	{
		while(T--)
		{
