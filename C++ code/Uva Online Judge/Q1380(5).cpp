...
//	printf("\tAt %d\n",u);
	...
	for(int i=0;i<GETO[u].size();i++)
	{
		...
		if(ud.up==INF||ud.up+ud.down+1>K)
		{
			ans.up=INF;
			return ans;
		}
		...
		else if(fa==-1)getmax(ans.down,ud.down+1);
		else v.push_back(ud);
	}
	return ans;
//	printf("%da: %d %d\n",u,ans.up,ans.down);
...
//				printf("%d: %d %d\n",j,F[j].down,F[j].up);
//			}
			if(ud.up!=INF&&ud.up+1+ud.down<=K)
			{
				found=true;
				break;
			}
...
{
	int t1,t2;
	if(type=='d')t1=1,t2=2,IN[to]++;
	else if(type=='u')t1=2,t2=1,IN[from]++;
	else t1=t2=0;
