...
double DP[100][12870];
int PRE[100][12870];
map<string,int> I,S_I;
map<int,string> S_NAME,Ir;
double FUND;
int D,S_N,S_LN;
char STR[9];
...
//			printf("PRICE[%d]=%.2lf\n",i,PRICE[0][i]);
		...
		for(int d=1;d<D;d++)
		{
			for(int i=0;i<I.size();i++)
			{
				double &dp=DP[d-1][i];
				if(dp<0)continue;
				if(getmax(DP[d][i],dp))PRE[d][i]=-1;
				for(int j=0;j<S_N;j++)
				{
					if(dp>=PRICE[d][j]&&can_buy(i,j))
					{
						//printf("%s to %s : %.2lf\n",Ir[i].c_str(),Ir[buy(i,j)].c_str(),dp);
						if(getmax(DP[d][buy(i,j)],dp-PRICE[d][j]))
						{
							PRE[d][buy(i,j)]=S_N+j;
						}
					}
					if(can_sell(i,j))
					{
						...
						if(getmax(DP[d][sell(i,j)],dp+PRICE[d][j]))
						{
							PRE[d][sell(i,j)]=j;
						}
					}
