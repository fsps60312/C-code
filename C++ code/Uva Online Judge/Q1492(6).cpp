...
//	freopen("in.txt","r",stdin);
	while(scanf("%lld%lld%lld%lld",&W,&H,&N,&M)==4)
	{
		...
		debug(printf("x.sz=(%d,%d),y.sz=(%d,%d)\n",XM.size(),X.size(),YM.size(),Y.size()););
		debug(for(LL i=0;i<X.size();i++)printf(" X%lld",X[i]);puts("");)
		debug(for(LL i=0;i<Y.size();i++)printf(" Y%lld",Y[i]);puts("");)
		for(LL i=0LL;i<N;i++)
		{
			Point &p=PP[i];
		...
		debug(for(LL i=0;i<P1.size();i++){Point p=P1[i];printf("P1:%lld %lld %lld %lld\n",p.x1,p.x2,p.y,p.t);});
		debug(for(LL i=0;i<P2.size();i++){Point p=P2[i];printf("P2:%lld %lld %lld %lld\n",p.x1,p.x2,p.y,p.t);});
		LL ans=0LL;
		if(W>=M)
		{
			LL sz=X.size()-1;
			...
			for(LL i=0LL,y1=1LL,y2;;)
			{
				...
				debug(printf("(%lld->%lld)%lld*%lld=%lld\n",y1,y2,(y2-y1+1LL),BT.Query(1LL,0LL,sz,X),(y2-y1+1LL)*BT.Query(1LL,0LL,sz,X)););
