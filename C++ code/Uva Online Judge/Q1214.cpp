...
{
	int *v,left;
	State():v(new int[M]),left(0){static int i;for(i=0;i<M;i++)v[i]=0;}
	int GetCode()
	{
		static int ans,i;
		ans=left;
...
int Solve(int r,int c,State &s)
{
	...
	{
		g--;
		if(s.TryMove(r,c,g,0,0,0,t))getmin(dp,Solve(r,c+1,t)+1);
		if(s.TryMove(r,c,0,g,0,0,t))getmin(dp,Solve(r,c+1,t)+1);
		if(s.TryMove(r,c,0,0,g,0,t))getmin(dp,Solve(r,c+1,t)+1);
		if(s.TryMove(r,c,0,0,0,g,t))getmin(dp,Solve(r,c+1,t)+1);
	}
	return dp;
}
int main()
	...
	while(scanf("%d%d",&N,&M)==2&&(N||M))
	{
		for(int i=0;i<N;i++)
		{
			for(int j=0;j<M;j++)
			{
				int &v=GRID[i][j];
				scanf("%d",&v);
				for(int k=0;k<POW[M+1];k++)DP[i][j][k]=-1;
			}
		}
		State s=State();
		int ans=Solve(0,0,s);
