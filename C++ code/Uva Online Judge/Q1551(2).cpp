...
struct Matrix
{
	...
	bool Solve(vector<int>&ans)
	{
		...
		for(int i=min(r,c)-1;i>=0;i--)for(int j=i-1;j>=0;j--)assert(s[i][i]&&EmptyRow(i,i)),Eliminate(i,j,i);
		for(int i=c;i<r;i++)
		{
			assert(EmptyRow(i));
			if(s[i][c])return false;
		}
		for(int i=0;i<min(r,c);i++)assert(s[i][i]&&EmptyRow(i,i));
		LL lca=1LL;
		for(int i=0;i<min(r,c);i++)lca=lca/Gcd(lca,labs(s[i][i]))*labs(s[i][i]);
		vector<int>up,down;
		for(int i=0;i<min(r,c);i++)
		{
