...
set<int> IN[MAXN];
int INSTED[MAXN],INSTIME_CNT,INSTIME[MAXN],EXPLICIT[MAXN];
...
void remove(int u)
{
	if(IN[u].size()){printf("   %s is still needed.\n",NAME[u].c_str());return;}
	...
	for(int i=0;i<LINK[u].size();i++)
	{
		int j=LINK[u][i];
		IN[j].erase(u);
		if(EXPLICIT[u]==EXPLICIT[j])
		{
			if(!IN[j].size())q.push((RPair){INSTIME[j],j});
			else EXPLICIT[j]=EXPLICIT[*IN[j].begin()];
		}
	}
	while(!q.empty())
	{
