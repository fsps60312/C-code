...
struct Hungary
{
	...
	void Update()
	{
		...
		assert(low!=INF);
//		printf("low=%lld\n",low);
		for(int i=0;i<M;i++)if(VA[i])WA[i]-=low;//,printf(" %d",i);puts("");
		for(int i=0;i<N;i++)if(VB[i])WB[i]+=low;//,printf(" %d",i);puts("");
//		for(int i=0;i<M;i++)printf(" %lld",WA[i]);puts("");
//		for(int i=0;i<N;i++)printf(" %lld",WB[i]);puts("");
	}
	void Solve()
	{
