...
int main()
{
//	freopen("C:\\Users\\user\\Downloads\\salesman\\salesman.030.in","r",stdin);
	...
	for(const auto &it0:SHOW)
	{
		...
		static vector<int>copy;copy.clear();
		for(int i=0;i<n;i++)copy.push_back(GetCost(loc[i]));
		tua[0]=0;
		mdc[0]=copy[0];
		for(int i=1,mn=copy[0];i<n;i++)
		{
			tua[i]=min(tua[i-1]+(loc[i]-loc[i-1])*(U+D)-earn[i-1],0);
			mn=mdc[i]=min(mn+Cost(loc[i-1],loc[i])-earn[i-1],copy[i]);
		}
		tda[n-1]=0;
		muc[n-1]=copy[n-1];
		for(int i=n-2,mn=copy[n-1];i>=0;i--)
		{
			tda[i]=min(tda[i+1]+(loc[i+1]-loc[i])*(U+D)-earn[i+1],0);
			mn=muc[i]=min(mn+Cost(loc[i+1],loc[i])-earn[i+1],copy[i]);
		}
		for(int i=0;i<n;i++)
		{
