...
//	freopen("in.txt","r",stdin);
	while(scanf("%d%d",&N,&K)==2)
	{
		...
		for(int i=0,j;i<N;)
		{
			...
			b.h=H[i];
			b.t=b.cnt=0;
			for(j=i;j<N&&H[j]==H[i];j++,b.cnt++)b.t|=(T[i]=='T');
			b.sum=j;
//			printf("%d:(h=%d,cnt=%d,t=%d,sum=%d)\n",i,b.h,b.cnt,b.t,b.sum);
			B.push_back(b);
			i=j;
		}
...
//		for(int i=0;i<B.size();i++)printf("%d:(h=%d,cnt=%d,t=%d,sum=%d)\n",i,B[i].h,B[i].cnt,B[i].t,B[i].sum);
		printf("%d\n",ans);
		break;
	}
	return 0;
