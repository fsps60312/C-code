...
int H,TYPE;
struct Cmp3{bool operator()(const int a,const int b)const{return TYPE==0?GetValue(H,a)>GetValue(H,b):rGetValue(H,a)<rGetValue(H,b);}};
bool rCmp2(const int a,const int b){return Cmp2(b,a);}
LL Solve3()
{
	LL ans=0LL;
	for(int &h=H=0;h<M;h++)
	{
		...
		for(int &type=TYPE=0;type<=1;type++)
		{
			sort(idx.begin(),idx.end(),type==0?Cmp2:rCmp2);
			SEG.Build(1,1,M*2-1);
			priority_queue<int,vector<int>,Cmp3>deq;
			for(auto i:idx)
			{
				while(!deq.empty()&&((type==0&&GetValue(H,deq.top())<X[i][0]+X[i][1])||(type==1&&rGetValue(H,deq.top())>X[i][0]+X[i][1])))
				{
				...
				if(type==0||(X[i][2]<h&&D-(h-X[i][2])>0))deq.push(i),SEG.Add(1,1,M*2-1,mid-d,mid+d,1);
			}
		}
	}
	return ans;
