...
//	for(int v:slopes)printf(" %d",v);puts("");
	for(int i=0;i<M-1;i++)
	{
		const LL v=(LL)slopes[i]*(LL)(RIDX[i+1]-RIDX[i]);
//		printf("y=%lld,i=%d,M-1=%d\n",y,i,M-1);
		if(v<0LL)y+=v;
		else return y;
	}
	return y;
...
//		if(true)
//		{
//			vector<int>tmp;SLOPE.Output(1,0,M-2,tmp);
//			for(int v:tmp)printf(" %d",v);puts("");
//		}
