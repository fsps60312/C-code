...
#include<map>
using namespace std;
const int MAX_BIG=50;
int N,R,Q;
int AREA[200000],BOSS[200000],DEP[200000];
vector<int>ET[200000],MEMBER[25000];
map<int,int>BIGA;
int UP[200000][MAX_BIG],DOWN[200000][MAX_BIG],ANS[MAX_BIG][MAX_BIG];
void BuildBIGA()
{
	BIGA.clear();
...
void BuildUDA(const int u,const int dep)
{
	DEP[u]=dep;
	FA[u].clear();
	if(u)
	...
	for(int i=0;i<(int)BIGA.size();i++)UP[u][i]=(u?UP[BOSS[u]][i]:0),DOWN[u][i]=0;
	const auto it=BIGA.find(AREA[u]);
	if(it!=BIGA.end())
	{
		for(int i=0;i<(int)BIGA.size();i++)ANS[i][it->second]+=UP[u][i];
		UP[u][it->second]++,DOWN[u][it->second]++;
	}
	for(const auto nxt:ET[u])
	{
		BuildUDA(nxt,dep+1);
		for(int i=0;i<(int)BIGA.size();i++)DOWN[u][i]+=DOWN[nxt][i];
	}
}
...
int Solve(const int r1,const int r2)
{
	...
	else if(it1==BIGA.end()&&it2!=BIGA.end())
	{
		for(const auto m:MEMBER[r1])ans+=DOWN[m][it2->second];
	}
	else if(it1!=BIGA.end()&&it2==BIGA.end())
	{
		for(const auto m:MEMBER[r2])ans+=UP[m][it1->second];
	}
...
int main()
{
	freopen("regions.006.IN","r",stdin);
