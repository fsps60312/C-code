...
int GetCnt(const int &l,const int &r)
{
	...
	for(;;)
	{
		if(idx>=DP.size())return ans;
		int i;
		for(i=-1;i+1<DP[idx].size()&&DP[idx][i+1]<=r;i++);
		if(i==-1)return ans;
		idx=DP[idx][i]+1;
		ans+=1<<i;
	}
...
//		}
		for(int i=0;i<N;i++)
		{
			...
			int v1=0,v2=0;
			assert((*it).l<=c.d1&&c.d2<=(*it).r);
			if((*it).l!=c.d1)v1=GetCnt((*it).l,c.d1-1);
			if(c.d2!=(*it).r)v2=GetCnt(c.d2+1,(*it).r);
			assert(v1+v2<=(*it).cnt-1);
			if(v1+v2!=(*it).cnt-1)continue;
//printf("\n%d(%d,%d)%d\n",i,(*it).l,(*it).r,(*it).cnt);
			int l=(*it).l,r=(*it).r;
			S.erase(it);
			if(v1)S.insert(Seg(l,c.d1-1,v1));
			if(v2)S.insert(Seg(c.d2+1,r,v2));
			printf("%d ",i+1);
			cnt++;
		}
		puts("");
