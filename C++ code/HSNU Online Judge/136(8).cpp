...
int Summary(const int u,const int s)
{
	int ans=DP[u][0];
	for(int a=s;a;a=(a-1)&s)getmax(ans,DP[u][a]);
	return ans;
}
void BuildDP(const int u,const int fa)
{
	...
	for(int s=0;s<=limit;s++)
	{
		if(s&(1<<upward))continue;
		getmax(DP[u][s],0);
		for(int i=0;i<CNT[u];i++)if(i!=upward&&!(s&(1<<i)))DP[u][s]+=MX[ET[u][i]];
		getmax(MX[u],DP[u][s]);
	}
	NOW[u]=Summary(u,limit-(1<<upward));
	for(int i=0;i<N;i++)if(BRANCH[i]!=upward)NOW[i]+=Summary(u,limit-(1<<upward)-(1<<BRANCH[i]))-MX[ET[u][BRANCH[i]]];
}
int Solve()
{
