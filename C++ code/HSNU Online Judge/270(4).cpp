...
map<int,int>BIGA;
map<int,int>UP[25000],DOWN[25000];
int ANS[MAX_BIG][MAX_BIG];
void BuildBIGA()
{
	BIGA.clear();
...
int Solve(const int r1,const int r2)
{
	...
	{
		const auto it=DOWN[r1].find(it2->second);
		return it==DOWN[r1].end()?0:it->second;
	}
	else if(it1!=BIGA.end()&&it2==BIGA.end())
	{
		const auto it=UP[r2].find(it1->second);
		return it==UP[r2].end()?0:it->second;
	}
...
//	freopen("out.txt","w",stdout);
	...
	for(int i=0;i<R;i++)UP[i].clear(),DOWN[i].clear();
	map<int,int>tmp;
	BuildUDA(0,0,tmp);
