...
int H;
struct Cmp3{bool operator()(const int a,const int b)const{return GetValue(H,a)>GetValue(H,b);}};
struct rCmp3{bool operator()(const int a,const int b)const{return rGetValue(H,a)<rGetValue(H,b);}};
...
#include<set>
struct _Cmp2{bool operator()(const int a,const int b)const{return Cmp2(a,b);}};
struct _rCmp2{bool operator()(const int a,const int b)const{return rCmp2(a,b);}};
void GoUp(const int h,LL &ans,multiset<int,_Cmp2>&idx)
{
	SEG.Build(1,1,M*2-1);
	priority_queue<int,vector<int>,Cmp3>deq;
	for(auto i:idx)
	{
		while(!deq.empty()&&GetValue(H,deq.top())<X[i][0]+X[i][1])
		{
			const int v=deq.top();deq.pop();
		...
		deq.push(i),SEG.Add(1,1,M*2-1,mid-d,mid+d,1);
	}
}
void GoDown(const int h,LL &ans,multiset<int,_rCmp2>idx)
{
	SEG.Build(1,1,M*2-1);
	priority_queue<int,vector<int>,rCmp3>deq;
	for(auto i:idx)
	{
		while(!deq.empty()&&rGetValue(H,deq.top())>X[i][0]+X[i][1])
		{
			const int v=deq.top();deq.pop();
			const int d=D-(h-X[v][2]);
			assert(d>=0);
		...
		const int mid=M+X[i][0]-X[i][1];
		if(X[i][2]==h)ans+=SEG.Query(1,1,M*2-1,mid);
		else deq.push(i),SEG.Add(1,1,M*2-1,mid-d,mid+d,1);
	}
}
bool CmpX2(const int a,const int b){return X[a][2]<X[b][2];}
LL Solve3()
{
	LL ans=0LL;
	vector<int>all;
	for(int i=0;i<N;i++)all.push_back(i);
	sort(all.begin(),all.end(),CmpX2);
	multiset<int,_Cmp2>idx1;
	multiset<int,_rCmp2>idx2;
	for(int &h=H=0,i_all=0;h<M;h++)
	{
		while(i_all<all.size()&&X[all[i_all]][2]==h)
		{
			const int i=all[i_all++];
			idx1.insert(i),idx2.insert(i);
		}
		GoUp(h,ans,idx1);
		GoDown(h,ans,idx2);
	}
