...
struct Bst
{
	...
	int Resize(const int id,const int l,const int r)
	{
		if(l==r)return id;
		const int mid=(l+r)/2;
		return max(Resize(id*2,l,mid),Resize(id*2+1,mid+1,r));
	}
	void Build(const int id,const int l,const int r,const LL &v,const int bound)
	{
		assert(id<(int)sum.size());
...
//	freopen("in.txt","r",stdin);
	...
	if(1)
	{
		const int maxid=BST.Resize(1,1,N+D);
		Resize(BST.sum,maxid+1);
		Resize(BST.lmx,maxid+1);
		Resize(BST.rmx,maxid+1);
		Resize(BST.cmx,maxid+1);
	}
