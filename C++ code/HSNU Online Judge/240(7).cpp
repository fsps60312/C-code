...
int LOC[1000][26];
bool USED[26];
bool Dfs(const int len,const int failed,const vector<int>&remain)
{
	if(failed>len)return true;//BUG on OJ
	if(remain.size()<=1)return false;//BUG on OJ
	for(int c=0;c<26;c++)
	{
		if(USED[c])continue;
		...
		USED[c]=true;
		bool win=false;
		for(int i=0;i<len;i++)if(win|=Dfs(len,failed,nxt[i]))break;
		if(!win)win|=Dfs(len,failed+1,nxt[len]);
		USED[c]=false;
		if(!win)return false;
	}
	return true;
