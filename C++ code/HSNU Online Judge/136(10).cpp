...
int OUTLET[1000][1000],EDGE_LOC[1000][5000];
vector<int>BuildOUTLET(const int u,const int parent)
{
	...
	{
		const vector<int>&ch_nodes=BuildOUTLET(nxt,u);
...
void BuildDP(const int u,const int parent,vector<int>&edges)
{
	...
	vector<int>updates;
	for(int s=0;s<=all;s++)if(dp[s])updates.push_back(s);
	vector<vector<int> >sets;
	sets.resize(n+1);
	for(int s=1;s<=all;s++)sets[CntBits(s)].push_back(s);
	for(int bitcnt=2;bitcnt<n;bitcnt++)
	{
		for(const int s:sets[bitcnt])
		{
			for(int i=0;i<n;i++)if(!(s&(1<<i)))getmax(dp[s+(1<<i)],dp[s]);
...
int main()
{
	...
	while(scanf("%d%d",&N,&M)==2)
	{
		...
		for(int i=0,a,b,c;i<M;i++)
		{
			...
			if(c!=0)edges.push_back(i),sum+=c;
		}
