...
//				printf("dest(%d):",dest.size());for(int i=0;i<dest.size();i++)printf(" %d",dest[i]);puts("");
//				printf("now (%d):",now.size());for(int i=0;i<now.size();i++)printf(" %d",now[i]);puts("");
				if(Better(dest,now))
				{
	...
	for(int i=1;i<10;i++)
	{
		vector<int>&dp=DP[N&1][i][Y];
		printf("dp[%d]:",i);for(int j=0;j<dp.size();j++)printf(" %d",dp[j]);puts("");
		if(!dp.empty()&&Better(ans,dp))ans=dp;
	}
