...
LL update1(Node1* &n,int x1,int x2,int x,int y,LL v)
{
	...
	{
		int midx=(x1+x2)/2;
		if(x<=midx)v=gcd(update1(n->cl,x1,midx,x,y,v),query1(n->cr,midx+1,x2,midx+1,x2,y,y));
		else v=gcd(update1(n->cr,midx+1,x2,x,y,v),query1(n->cl,x1,midx,x1,midx,y,y));
	}
	return update2(n->cud,0,C,y,v);
}
int main()
{
