...
void BuildDown(const int r1,int *ans)
{
	...
	for(const int m:MEMBER[r1])change[INTV[m][0]]++,change[INTV[m][1]+1]--;
...
int LRD[500][25000],LRU[500][25000],BSZ;
map<int,int>BIG;
...
//	{
//		sprintf(filename,"D:\\Users\\Burney\\Downloads\\regions\\regions.%03d.in",fileidx);
//		freopen(filename,"r",stdin);
//		freopen(filename+34,"w",stdout);
		assert(scanf("%d%d%d",&N,&R,&Q)==3);
		assert(N>=1&&N<=200000&&R>=1&&R<=25000&&Q>=1);
		for(int i=0;i<R;i++)MEMBER[i].clear();
		for(int i=0;i<N;i++)ET[i].clear();
		assert(INPUT.empty());
		...
		do
		{
			static multimap<int,int,greater<int> >sot;sot.clear();
