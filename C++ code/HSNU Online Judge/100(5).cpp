...
void Solve()
{
	...
	int head=GetHead();
	for(int i=0;i<N;i++)if(BCC[IDX[Find(i)]]!=head)ANS[i]=false;
}
set<int>S;
int main()
{
	...
	for(int i=0,cnt,v;i<N;i++)
	{
		...
		{
			if(EO[i].find(*it)!=EO[i].end())continue;
			if(EO[*it].find(Find(i))!=EO[*it].end())tomerge.push_back(Find(i));
		}
	...
	for(auto i1=S.begin();i1!=S.end();i1++,M++)
	{
		IDX[*i1]=M;
		ET[M].clear();
		for(auto i2=EO[*i1].begin();i2!=EO[*i1].end();i2++)ET[M].push_back(*i2);
	}
	for(int i=0;i<N;i++)
	{
		printf("%d->%d\n",i,IDX[Find(i)]);
