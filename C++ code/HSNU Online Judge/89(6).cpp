...
//	for(int i=0;i<10;i++)printf("edge %d:(%d,%d)\n",i,ET[4][i].u,ET[4][i].c);
	V[0].push_back(0);
...
//		printf("d=%d\n",d);
		for(int s=0;s<V[d].size();s++)
		{
			...
			vector<int>&now=DP[d][u%10][orig][cost];
//			printf("u=%d,orig=%d,cost=%d,now=",u%10,orig,cost);for(int i=(int)now.size()-1;i>=0;i--)printf("%d",now[i]);puts("");
			assert(now.size()==digitloc-1);
			for(int i=(digitloc==N?1:0);i<10;i++)
			{
				now.push_back(i);
				const Edge &e=ET[u%10][i];
				if(orig+i>X)continue;
				const int nxt=(orig+i)*10*1000+(cost+e.c)*10+e.u;
				vector<int>&dest=DP[d^1][e.u][orig+i][cost+e.c];
				assert(now.size()==digitloc);
				if(Better(dest,now))
				{
					if(dest.empty())V[d^1].push_back(nxt);
					dest=now;
					assert(dest.size()==digitloc);
				}
				now.pop_back();
			}
			now.clear();
