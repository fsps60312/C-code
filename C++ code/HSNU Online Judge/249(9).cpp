...
//		printf("l==%d\n",l);
		...
		int ans=0;
		for(int rank=K-1,r=K-1;rank>=0;rank--)
		{
			const int c=COLOR[rank];
			while(l>=0&&FISH[l].len*2>MXLEN[c])AddFish(FISH[l--],-1);
			printf("cnt:");for(int i=0;i<K;i++)printf(" %d",CNT[RANK[i]]);puts("");
			if(CNT[rank]+1>MX[rank])
			{
//				while(MXLEN[COLOR[r]]>=MXLEN[c]*2)r--;
//				printf("r=%d,MXLEN[]=(%d,%d)\n",r,MXLEN[COLOR[r]],MXLEN[c]);
				int ta=ans;
				ans=Plus(ans,Mul(SEG.Query(0,rank-1),SEG.Query(rank+1,K-1)));
				printf("rank (%d,%d): extra %d\n",rank,c+1,ans-ta);
				ta=ans;
				ans=Plus(ans,Mul(SEG.Query(0,rank-1),CNT[rank]));
				printf("rank (%d,%d): cost %d\n",rank,c+1,ans-ta);
			}
//			else if(CNT[rank]+1==MX[rank])
//			{
//				while(FISH[r].len>=MXLEN[c]*2)r--;
//				int ta=ans;
//				ans=Plus(ans,Mul(SEG.Query(0,rank-1),SEG.Query(rank+1,r)));
//				printf("rank (%d,%d): cost %d\n",rank,c+1,ans-ta);
//			}
			else
			{
