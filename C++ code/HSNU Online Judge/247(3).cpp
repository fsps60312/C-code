...
void MarkVIS(const int u,const int fa,const int a,const int b)
{
	if(u==a||u==b)return;
	assert(!VIS[u]);
	VIS[u]=true;
	for(int i=0;i<ET[u].size();i++)if(ET[u][i]!=fa)MarkVIS(EDGE[ET[u][i]].Nxt(u),ET[u][i],a,b);
}
LL GetDiameter(const int source)
{
	...
	for(int i=0;i<ucycle.size();i++)assert(!VIS[ucycle[i]]),VIS[ucycle[i]]=true;
	vector<LL>h,gap;
	gap.push_back(0LL);
	...
	for(int i=0;i<h.size();i++)
	{
		...
		if(!DEQ.empty())getmax(ta,h[i]+gap[i]-gap[DEQ.front()]+h[DEQ.front()]);
		DEQ.push_back(i);
	}
	for(int i=0;i<ucycle.size();i++)
	{
		const int u=ucycle[i];
		VIS[u]=false;
		MarkVIS(u,-1,ucycle[(i+1)%ucycle.size()],ucycle[(i+ucycle.size()-1)%ucycle.size()]);
	}
...
LL Solve()
{
	...
	LL ans=0LL;
	for(int u=0;u<N;u++)if(!VIS[u])ans+=GetDiameter(u);
	return ans;
}
int main()
{
