...
bool Dfs(const int u,const int c)
{
	...
	if(VIS[u]){assert(COLOR[u]==c);return true;}
	VIS[u]=true;
	assert(COLOR[u]==-1||COLOR[u]==c);
	CHANGE.push(Change(u,COLOR[u]));
	COLOR[u]=c;
	for(int i=0;i<ET[u].size();i++)
	{
...
//		printf("expcolor=%d\n",expcolor);
		...
		if(!Dfs(nxt,(expcolor^d.c)^c))return false;
	}
	return true;
}
int main()
...
//	freopen("in.txt","r",stdin);
	scanf("%d%d%d",&N,&M,&K);
	for(int i=0;i<N+M;i++)COLOR[i]=-1,ET[i].clear(),VIS[i]=false;
	vector<int>didx;
	for(int i=0;i<K;i++)
	{
...
//	for(int i=0;i<N+M;i++)assert(!VIS[i]);
	printf("%lld\n",ans);
