	...
	while(scanf("%d%d",&N,&K)==2)
	{
		...
		if(texist)
		{
			...
			for(int i=0,j;i<B.size();i++)
			{
				...
				{
					VIS[i]=true;
					for(j=i-1;j>=0&&!VIS[j]&&B[j].h>B[j+1].h;j--)VIS[j]=true;
					for(j=i+1;j<B.size()&&!VIS[j]&&B[j].h>B[j-1].h;j++)VIS[j]=true;
				}
			}
			for(int i=0,j;i<B.size();i++)
			{
				...
				else if((i==0||B[i-1].h<B[i].h)&&(i==B.size()-1||B[i+1].h<B[i].h))
				{
					for(j=i-1;j>=0&&!VIS[j]&&B[j].h<B[j+1].h;j--);j++;if(j!=i)mx=max(mx,Sum(j,VIS[i]?i-1:i));
					for(j=i+1;j<B.size()&&!VIS[j]&&B[j].h<B[j-1].h;j++);j--;if(j!=i)mx=max(mx,Sum(VIS[i]?i+1:i,j));
				}
			}
			int ta=0;
